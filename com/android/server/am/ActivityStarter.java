...
class ActivityStarter {
	...
	// TODO b/30204367 remove when the platform fully supports ephemeral
	// applications
	...
	// Indicates that we moved other task and are going to put something on top
	// soon, so
	// we don't want to show it redundantly or accidentally change what's shown
	// below.
	...
	final int startActivityLocked(IApplicationThread caller, Intent intent, Intent ephemeralIntent, String resolvedType,
			ActivityInfo aInfo, ResolveInfo rInfo, IVoiceInteractionSession voiceSession,
			IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode, int callingPid,
			int callingUid, String callingPackage, int realCallingPid, int realCallingUid, int startFlags,
		...
		if (caller != null) {
			...
			if (callerApp != null) {
				...
				Slog.w(TAG, "Unable to find app for caller " + caller + " (pid=" + callingPid + ") when starting: "
						+ intent.toString());
		...
		if (err == ActivityManager.START_SUCCESS) {
			Slog.i(TAG,
					"START u" + userId + " {" + intent.toShortString(true, true, true, false) + "} from uid "
							+ callingUid + " on display "
							+ (container == null
									? (mSupervisor.mFocusedStack == null ? Display.DEFAULT_DISPLAY
											: mSupervisor.mFocusedStack.mDisplayId)
		...
		if (err == ActivityManager.START_SUCCESS && sourceRecord != null && sourceRecord.task.voiceSession != null) {
			...
			if ((launchFlags & FLAG_ACTIVITY_NEW_TASK) == 0
				...
				try {
					...
					if (!AppGlobals.getPackageManager().activitySupportsIntent(intent.getComponent(), intent,
							resolvedType)) {
		...
		boolean abort = !mSupervisor.checkStartAnyActivityPermission(intent, aInfo, resultWho, requestCode, callingPid,
				callingUid, callingPackage, ignoreTargetSecurity, callerApp, resultRecord, resultStack, options);
		abort |= !mService.mIntentFirewall.checkStartActivity(intent, callingUid, callingPid, resolvedType,
		...
		if (Build.PERMISSIONS_REVIEW_REQUIRED && aInfo != null) {
			if (mService.getPackageManagerInternalLocked().isPermissionsReviewRequired(aInfo.packageName, userId)) {
				IIntentSender target = mService.getIntentSenderLocked(ActivityManager.INTENT_SENDER_ACTIVITY,
						callingPackage, callingUid, userId, null, null, 0, new Intent[] { intent },
						new String[] { resolvedType }, PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_ONE_SHOT,
						null);
		...
		// If we have an ephemeral app, abort the process of launching the resolved
		// intent.
		// Instead, launch the ephemeral installer. Once the installer is finished, it
		// starts either the intent we resolved here [on install error] or the ephemeral
		// app [on install success].
		if (rInfo != null && rInfo.ephemeralResolveInfo != null) {
			intent = buildEphemeralInstallerIntent(intent, ephemeralIntent, rInfo.ephemeralResolveInfo.getPackageName(),
		...
		ActivityRecord r = new ActivityRecord(mService, callerApp, callingUid, callingPackage, intent, resolvedType,
				aInfo, mService.mConfiguration, resultRecord, resultWho, requestCode, componentSpecified,
		...
		if (voiceSession == null
				&& (stack.mResumedActivity == null || stack.mResumedActivity.info.applicationInfo.uid != callingUid)) {
			if (!mService.checkAppSwitchAllowedLocked(callingPid, callingUid, realCallingPid, realCallingUid,
					"Activity start")) {
		...
		try {
			...
			err = startActivityUnchecked(r, sourceRecord, voiceSession, voiceInteractor, startFlags, true, options,
					inTask);
		} finally {
	 ...
	 */
	private Intent buildEphemeralInstallerIntent(Intent launchIntent, Intent origIntent, String ephemeralPackage,
			String callingPackage, String resolvedType, int userId) {
		...
		// for any reason.
		final IIntentSender failureIntentTarget = mService.getIntentSenderLocked(ActivityManager.INTENT_SENDER_ACTIVITY,
				callingPackage, Binder.getCallingUid(), userId, null /* token */, null /* resultWho */, 1,
		...
		// broadcast receiver.
		final IIntentSender successIntentTarget = mService.getIntentSenderLocked(ActivityManager.INTENT_SENDER_ACTIVITY,
				callingPackage, Binder.getCallingUid(), userId, null /* token */, null /* resultWho */, 0,
		...
		intent.setFlags(flags | Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK
	...
	void postStartActivityUncheckedProcessing(ActivityRecord r, int result, int prevFocusedStackId,
			ActivityRecord sourceRecord, ActivityStack targetStack) {

		if (result < START_SUCCESS) {
			// If someone asked to have the keyguard dismissed on the next activity start,
			// but we are not actually doing an activity switch... just dismiss the keyguard
			// now,
		...
		// If we launched the activity from a no display activity that was launched from
		// the home
		// screen, we also need to start recents to un-minimize the docked stack, since
		// the
		// noDisplay activity will be finished shortly after.
		// Note that some apps have trampoline activities without noDisplay being set.
		// In that case,
		// we have another heuristic in
		// DockedStackDividerController.notifyAppTransitionStarting
		// that tries to detect that case.
		// TODO: We should prevent noDisplay activities from affecting task/stack
		// ordering and
		// visibility instead of using this flag.
		final boolean noDisplayActivityOverHome = sourceRecord != null && sourceRecord.noDisplay
		...
		if (startedActivityStackId == DOCKED_STACK_ID
			...
			if (topActivityHomeStack == null || topActivityHomeStack.mActivityType != RECENTS_ACTIVITY_TYPE) {
				// We launch an activity while being in home stack, which means either launcher
				// or
				// recents into docked stack. We don't want the launched activity to be alone in
				// a
		...
		if (startedActivityStackId == PINNED_STACK_ID
			...
			// The activity was already running in the pinned stack so it wasn't started,
			// but either
			// brought to the front or the new intent was delivered to it since it was
			// already in
	...
	void startHomeActivityLocked(Intent intent, ActivityInfo aInfo, String reason) {
		...
		startActivityLocked(null /* caller */, intent, null /* ephemeralIntent */, null /* resolvedType */, aInfo,
				null /* rInfo */, null /* voiceSession */, null /* voiceInteractor */, null /* resultTo */,
				null /* resultWho */, 0 /* requestCode */, 0 /* callingPid */, 0 /* callingUid */,
				null /* callingPackage */, 0 /* realCallingPid */, 0 /* realCallingUid */, 0 /* startFlags */,
				null /* options */, false /* ignoreTargetSecurity */, false /* componentSpecified */,
				null /* outActivity */, null /* container */, null /* inTask */);
		if (mSupervisor.inResumeTopActivity) {
			// If we are in resume section already, home activity will be initialized, but
			// not
			// resumed (to avoid recursive resume) and will stay that way until something
			// pokes it
	...
	void showConfirmDeviceCredential(int userId) {
		// First, retrieve the stack that we want to resume after credential is
		// confirmed.
		...
		if (fullscreenStack != null
			...
			// Single window case and the case that the docked stack is shown with
			// fullscreen stack.
		...
		// For safety, check null here in case users changed the setting after the
		// checking.
		if (credential == null) {
			return;
		}
		...
		if (activityRecord != null) {
			final IIntentSender target = mService.getIntentSenderLocked(ActivityManager.INTENT_SENDER_ACTIVITY,
					activityRecord.launchedFromPackage, activityRecord.launchedFromUid, activityRecord.userId, null,
					null, 0, new Intent[] { activityRecord.intent }, new String[] { activityRecord.resolvedType },
					PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_IMMUTABLE,
					null);
	...
	final int startActivityMayWait(IApplicationThread caller, int callingUid, String callingPackage, Intent intent,
			String resolvedType, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor,
			IBinder resultTo, String resultWho, int requestCode, int startFlags, ProfilerInfo profilerInfo,
			IActivityManager.WaitResult outResult, Configuration config, Bundle bOptions, boolean ignoreTargetSecurity,
		...
		synchronized (mService) {
			...
			if (aInfo != null
					&& (aInfo.applicationInfo.privateFlags & ApplicationInfo.PRIVATE_FLAG_CANT_SAVE_STATE) != 0) {
				...
				if (aInfo.processName.equals(aInfo.applicationInfo.packageName)) {
					...
					if (heavy != null && (heavy.info.uid != aInfo.applicationInfo.uid
						...
						IIntentSender target = mService.getIntentSenderLocked(ActivityManager.INTENT_SENDER_ACTIVITY,
								"android", appCallingUid, userId, null, null, 0, new Intent[] { intent },
			...
			// --------------startActivityLocked 开始启动------------>
			int res = startActivityLocked(caller, intent, ephemeralIntent, resolvedType, aInfo, rInfo, voiceSession,
					voiceInteractor, resultTo, resultWho, requestCode, callingPid, callingUid, callingPackage,
					realCallingPid, realCallingUid, startFlags, options, ignoreTargetSecurity, componentSpecified,
	...
	final int startActivities(IApplicationThread caller, int callingUid, String callingPackage, Intent[] intents,
		...
		try {
			synchronized (mService) {
				...
				for (int i = 0; i < intents.length; i++) {
					...
					int res = startActivityLocked(caller, intent, null /* ephemeralIntent */, resolvedTypes[i], aInfo,
							null /* rInfo */, null, null, resultTo, null, -1, callingPid, callingUid, callingPackage,
							realCallingPid, realCallingUid, 0, options, false, componentSpecified, outActivity, null,
							null);
					if (res < 0) {
						return res;
					}
	...
	void sendPowerHintForLaunchStartIfNeeded(boolean forceSend) {
		// Trigger launch power hint if activity being launched is not in the current
		// task
		final ActivityStack focusStack = mSupervisor.getFocusedStack();
		final ActivityRecord curTop = (focusStack == null) ? null
				: focusStack.topRunningNonDelayedActivityLocked(mNotTop);
		if ((forceSend || (!mPowerHintSent && curTop != null && curTop.task != null && mStartActivity != null
				&& curTop.task != mStartActivity.task)) && mService.mLocalPowerManager != null) {
	...
	private int startActivityUnchecked(final ActivityRecord r, ActivityRecord sourceRecord,
			IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, int startFlags, boolean doResume,
		...
		if (mReusedActivity != null) {
			...
			// still needs to be a lock task mode violation since the task gets cleared out
			// and
			// the device would otherwise leave the locked task.
			if (mSupervisor.isLockTaskModeViolation(mReusedActivity.task,
					(mLaunchFlags & (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TASK)) == (FLAG_ACTIVITY_NEW_TASK
			...
			// This code path leads to delivering a new intent, we want to make sure we
			// schedule it
			// as the first operation, in case the activity will be resumed as a result of
			// later
			// operations.
			if ((mLaunchFlags & FLAG_ACTIVITY_CLEAR_TOP) != 0 || mLaunchSingleInstance || mLaunchSingleTask) {
				// In this situation we want to remove all activities from the task up to the
				// one
				// being started. In most cases this means we are resetting the task to its
				// initial
				// state.
				final ActivityRecord top = mReusedActivity.task.performClearTaskForReuseLocked(mStartActivity,
						mLaunchFlags);
				if (top != null) {
					if (top.frontOfTask) {
		...
		// If the activity being launched is the same as the one currently at the top,
		// then
		...
		final boolean dontStart = top != null && mStartActivity.resultTo == null
				...
				&& ((mLaunchFlags & FLAG_ACTIVITY_SINGLE_TOP) != 0 || mLaunchSingleTop || mLaunchSingleTask);
		if (dontStart) {
			ActivityStack.logStartActivity(AM_NEW_INTENT, top, top.task);
			// For paranoia, make sure we have correctly resumed the top activity.
			topStack.mLastPausedActivity = null;
			if (mDoResume) {
				//--------------------接下来会暂停顶部acrivtiy，并为新的应用创建进程------------------------
		...
		if (mStartActivity.resultTo == null && mInTask == null && !mAddingToTask
			...
			if (!mMovedOtherTask) {
				// If stack id is specified in activity options, usually it means that activity
				// is
				// launched not from currently focused stack (e.g. from SysUI or from shell) -
				// in
			...
			// This not being started from an existing activity, and not part of a new
			// task...
			// just put it in the top task, though these days this case should never happen.
			setTaskToCurrentTopOrCreateNewTask();
		}

		mService.grantUriPermissionFromIntentLocked(mCallingUid, mStartActivity.packageName, mIntent,
				mStartActivity.getUriPermissionsLocked(), mStartActivity.userId);
		...
		if (mDoResume) {
			...
			if (!mTargetStack.isFocusable()
				...
				// Also, we don't want to resume activities in a task that currently has an
				// overlay
	...
	private void setInitialState(ActivityRecord r, ActivityOptions options, TaskRecord inTask, boolean doResume,
			int startFlags, ActivityRecord sourceRecord, IVoiceInteractionSession voiceSession,
		...
		mLaunchTaskBehind = r.mLaunchTaskBehind && !mLaunchSingleTask && !mLaunchSingleInstance
		...
		// If we are actually going to launch in to a new task, there are some cases
		// where
		...
		if (DEBUG_USER_LEAVING)
			Slog.v(TAG_USER_LEAVING, "startActivity() => mUserLeaving=" + mSupervisor.mUserLeaving);
		...
		mInTask = inTask;
		// In some flows in to this function, we retrieve the task record and hold on to
		// it
		// without a lock before calling back in to here... so the task at this point
		// may
		...
		// If the onlyIfNeeded flag is set, then we can do this if the activity being
		// launched
		// is the same as the one making the call... or, as a special case, if we do not
		// know
	...
	private void sendNewTaskResultRequestIfNeeded() {
		if (mStartActivity.resultTo != null && (mLaunchFlags & FLAG_ACTIVITY_NEW_TASK) != 0
			...
			// so instead immediately send back a cancel and let the new task continue
			// launched
	...
	private void computeLaunchingTaskFlags() {
		...
		// explicit task into
		// which they would like us to launch the new activity, then let's see about
		// doing that.
		if (mSourceRecord == null && mInTask != null && mInTask.stack != null) {
			...
			if (mLaunchSingleInstance || mLaunchSingleTask) {
				if (!baseIntent.getComponent().equals(mStartActivity.intent.getComponent())) {
					ActivityOptions.abort(mOptions);
					throw new IllegalArgumentException("Trying to launch singleInstance/Task " + mStartActivity
							+ " into different task " + mInTask);
				}
				if (root != null) {
					ActivityOptions.abort(mOptions);
					throw new IllegalArgumentException("Caller with mInTask " + mInTask + " has root " + root
			...
			// Also put noDisplay activities in the source task. These by itself can be
			// placed
			// in any task/stack, however it could launch other activities like
			// ResolverActivity,
	...
	private void computeSourceStack() {
		...
		// If the source is finishing, we can't further count it as our source. This is
		// because the
		// task it is associated with may now be empty and on its way out, so we don't
		// want to
		// blindly throw it in to that task. Instead we will take the NEW_TASK flow and
		// try to find
		// a task for it. But save the task information so it can be used when creating
		// the new task.
		if ((mLaunchFlags & FLAG_ACTIVITY_NEW_TASK) == 0) {
			Slog.w(TAG, "startActivity called from finishing " + mSourceRecord + "; forcing "
					+ "Intent.FLAG_ACTIVITY_NEW_TASK for: " + mIntent);
			mLaunchFlags |= FLAG_ACTIVITY_NEW_TASK;
	 ...
	 * Returns null if not or an ActivityRecord with the task into which the new
	 * activity should be added.
	 */
	private ActivityRecord getReusableIntentActivity() {
		// We may want to try to place the new activity in to an existing task. We
		// always
		// do this if the target activity is singleTask or singleInstance; we will also
		// do
		// this if NEW_TASK has been requested, and there is not an additional qualifier
		// telling
		// us to still place it in a new task: multi task, always doc mode, or being
		// asked to
		// launch this as a new task behind the current one.
		boolean putIntoExistingTask = ((mLaunchFlags & FLAG_ACTIVITY_NEW_TASK) != 0
				&& (mLaunchFlags & FLAG_ACTIVITY_MULTIPLE_TASK) == 0) || mLaunchSingleInstance || mLaunchSingleTask;
		// If bring to front is requested, and no result is requested and we have not
		// been given
		// an explicit task to launch in to, and we can find a task that was started
		// with this
	...
	private ActivityRecord setTargetStackAndMoveToFrontIfNeeded(ActivityRecord intentActivity) {
		...
		mTargetStack.mLastPausedActivity = null;
		// If the target task is not in the front, then we need to bring it to the
		// front...
		// except... well, with SINGLE_TASK_LAUNCH it's not entirely clear. We'd like to
		// have
		// the same behavior as if a new instance was being started, which means not
		// bringing it
		...
		if (curTop != null && (curTop.task != intentActivity.task || curTop.task != focusStack.topTask())
			...
			if (mSourceRecord == null
					|| (mSourceStack.topActivity() != null && mSourceStack.topActivity().task == mSourceRecord.task)) {
				...
				// Here we only want to bring the target stack forward. Transition will be
				// applied
				// to the new activity that's started after the old ones are gone.
				final boolean willClearTask = (mLaunchFlags
						& (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TASK)) == (FLAG_ACTIVITY_NEW_TASK
								| FLAG_ACTIVITY_CLEAR_TASK);
				if (!willClearTask) {
					final ActivityStack launchStack = getLaunchStack(mStartActivity, mLaunchFlags, mStartActivity.task,
							mOptions);
					if (launchStack == null || launchStack == mTargetStack) {
						// We only want to move to the front, if we aren't going to launch on a
						// different stack. If we launch on a different stack, we will put the
						// task on top there.
						mTargetStack.moveTaskToFrontLocked(intentActivity.task, mNoAnimation, mOptions,
						...
						if ((mLaunchFlags & FLAG_ACTIVITY_LAUNCH_ADJACENT) != 0) {
							...
							mSupervisor.moveTaskToStackLocked(intentActivity.task.taskId, launchStack.mStackId, ON_TOP,
							...
							mTargetStack.moveTaskToFrontLocked(intentActivity.task, mNoAnimation, mOptions,
									mStartActivity.appTimeTracker, "bringToFrontInsteadOfAdjacentLaunch");
						}
						mMovedToFront = true;
					}
					mOptions = null;
		...
		if (!mMovedToFront && mDoResume) {
			if (DEBUG_TASKS)
				Slog.d(TAG_TASKS, "Bring to front target: " + mTargetStack + " from " + intentActivity);
			mTargetStack.moveToFront("intentActivityFound");
		}

		mSupervisor.handleNonResizableTaskIfNeeded(intentActivity.task, INVALID_STACK_ID, mTargetStack.mStackId);
	...
	private void updateTaskReturnToType(TaskRecord task, int launchFlags, ActivityStack focusedStack) {
		if ((launchFlags & (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_TASK_ON_HOME)) == (FLAG_ACTIVITY_NEW_TASK
	...
	private void setTaskFromIntentActivity(ActivityRecord intentActivity) {
		if ((mLaunchFlags & (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_CLEAR_TASK)) == (FLAG_ACTIVITY_NEW_TASK
				| FLAG_ACTIVITY_CLEAR_TASK)) {
			...
			// When we clear the task - focus will be adjusted, which will bring another
			// task
			...
			if (top == null) {
				...
				if (task != null && task.stack == null) {
					...
					mTargetStack = computeStackFocus(mSourceRecord, false /* newTask */, null /* bounds */,
			...
			// In this case the top activity on the task is the same as the one being
			// launched,
			// so we take that as a request to bring the task to the foreground. If the top
			// activity in the task is the root activity, deliver this new intent to it if
			// it
			// desires.
			...
			// from a notification or shortcut. We want to place the new activity on top of
			// the
			// current task.
			mAddingToTask = true;
			mSourceRecord = intentActivity;
		} else if (!intentActivity.task.rootWasReset) {
			// In this case we are launching into an existing task that has not yet been
			// started
			// from its front door. The current task has been brought to the front. Ideally,
			// we'd probably like to place this new task at the bottom of its stack, but
			// that's
			// a little hard to do with the current organization of the code so for now
			// we'll
			// just drop it.
			intentActivity.task.setIntent(mStartActivity);
		}
	...
	private void setTaskFromReuseOrCreateNewTask(TaskRecord taskToAffiliate) {
		...
		if (mReuseTask == null) {
			final TaskRecord task = mTargetStack.createTaskRecord(
					...
					mNewTaskIntent != null ? mNewTaskIntent : mIntent, mVoiceSession, mVoiceInteractor,
			...
			if (DEBUG_TASKS)
				Slog.v(TAG_TASKS, "Starting new activity " + mStartActivity + " in new task " + mStartActivity.task);
		} else {
	...
	private int setTaskFromSourceRecord() {
		...
		// otherwise we would move the launching task to the other side, rather than
		// show
		// two side by side.
		...
		if (mTargetStack == null) {
			...
			mSupervisor.moveTaskToStackLocked(sourceTask.taskId, mTargetStack.mStackId, ON_TOP, FORCE_FOCUS,
					"launchToSide", !ANIMATE);
		}
		if (mDoResume) {
		...
		if (topTask != sourceTask && !mAvoidMoveToFront) {
			mTargetStack.moveTaskToFrontLocked(sourceTask, mNoAnimation, mOptions, mStartActivity.appTimeTracker,
					"sourceTaskToFront");
		}
		if (!mAddingToTask && (mLaunchFlags & FLAG_ACTIVITY_CLEAR_TOP) != 0) {
			// In this case, we are adding the activity to an existing task, but the caller
			// has
			...
			// be
			// running somewhere in the history, and we want to shuffle it to the front of
			// the
			// stack if so.
		...
		// An existing activity is starting this new activity, so we want to keep the
		// new one in
		// the same task as the one that is starting it.
		mStartActivity.setTask(sourceTask, null);
		if (DEBUG_TASKS)
			Slog.v(TAG_TASKS, "Starting new activity " + mStartActivity + " in existing task " + mStartActivity.task
	...
	private int setTaskFromInTask() {
		if (mLaunchBounds != null) {
			...
			if (stackId != mInTask.stack.mStackId) {
				final ActivityStack stack = mSupervisor.moveTaskToStackUncheckedLocked(mInTask, stackId, ON_TOP,
						!FORCE_FOCUS, "inTaskToFront");
		...
		mTargetStack.moveTaskToFrontLocked(mInTask, mNoAnimation, mOptions, mStartActivity.appTimeTracker,
				"inTaskToFront");
		...
		if (DEBUG_TASKS)
			Slog.v(TAG_TASKS, "Starting new activity " + mStartActivity + " in explicit task " + mStartActivity.task);

		return START_SUCCESS;
	}

	private void setTaskToCurrentTopOrCreateNewTask() {
		mTargetStack = computeStackFocus(mStartActivity, false, null /* bounds */, mLaunchFlags, mOptions);
		if (mDoResume) {
			mTargetStack.moveToFront("addingToTopTask");
		}
		final ActivityRecord prev = mTargetStack.topActivity();
		final TaskRecord task = (prev != null) ? prev.task
				: mTargetStack.createTaskRecord(mSupervisor.getNextTaskIdForUserLocked(mStartActivity.userId),
		...
		if (DEBUG_TASKS)
			Slog.v(TAG_TASKS, "Starting new activity " + mStartActivity + " in new guessed " + mStartActivity.task);
	}
	...
	private int adjustLaunchFlagsToDocumentMode(ActivityRecord r, boolean launchSingleInstance,
		...
		if ((launchFlags & Intent.FLAG_ACTIVITY_NEW_DOCUMENT) != 0 && (launchSingleInstance || launchSingleTask)) {
			// We have a conflict between the Intent and the Activity manifest, manifest
			// wins.
	...
	final void doPendingActivityLaunchesLocked(boolean doResume) {
		while (!mPendingActivityLaunches.isEmpty()) {
			...
			try {
				final int result = startActivityUnchecked(pal.r, pal.sourceRecord, null, null, pal.startFlags, resume,
						null, null);
				postStartActivityUncheckedProcessing(pal.r, result, mSupervisor.mFocusedStack.mStackId, mSourceRecord,
						mTargetStack);
			} catch (Exception e) {
	...
	private ActivityStack computeStackFocus(ActivityRecord r, boolean newTask, Rect bounds, int launchFlags,
		...
		if (task != null && task.stack != null) {
			...
			if (stack.isOnHomeDisplay()) {
				if (mSupervisor.mFocusedStack != stack) {
					...
					if (DEBUG_FOCUS || DEBUG_STACK)
						Slog.d(TAG_FOCUS, "computeStackFocus: Focused stack already=" + mSupervisor.mFocusedStack);
				}
			}
			return stack;
		}
		...
		// resizeable
		// or not. So, we let the task go in the fullscreen task if it is the focus
		// stack.
		// If the freeform or docked stack has focus, and the activity to be launched is
		// resizeable,
		...
		if (canUseFocusedStack && (!newTask || mSupervisor.mFocusedStack.mActivityContainer.isEligibleForNewTasks())) {
			if (DEBUG_FOCUS || DEBUG_STACK)
				Slog.d(TAG_FOCUS, "computeStackFocus: Have a focused stack=" + mSupervisor.mFocusedStack);
		...
		// If there is no suitable dynamic stack then we figure out which static stack
		// to use.
		final int stackId = task != null ? task.getLaunchStackId()
				: bounds != null ? FREEFORM_WORKSPACE_STACK_ID : FULLSCREEN_WORKSPACE_STACK_ID;
	...
	private ActivityStack getLaunchStack(ActivityRecord r, int launchFlags, TaskRecord task, ActivityOptions aOptions) {
		...
		if (isValidLaunchStackId(launchStackId, r)) {
			...
			// The preferred launch stack is the docked stack, but it isn't a valid launch
			// stack
		...
		final ActivityStack parentStack = task != null ? task.stack
				: r.mInitialActivityContainer != null ? r.mInitialActivityContainer.mStack : mSupervisor.mFocusedStack;
		...
		if (parentStack != mSupervisor.mFocusedStack) {
			...
			if (mSupervisor.mFocusedStack != null && task == mSupervisor.mFocusedStack.topTask()) {
				// If task is already on top of focused stack - use it. We don't want to move
				// the
				// existing focused task to adjacent stack, just deliver new intent in this
				// case.
				return mSupervisor.mFocusedStack;
			}
