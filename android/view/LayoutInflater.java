...
public abstract class LayoutInflater {
    ...
    View createViewFromTag(View parent, String name, Context context, AttributeSet attrs,
        ...
        try {
            ...
            if (view == null) {
                ...
                try {// 内置View控件，因为只有自定义控件才会有全限定类名，如：android.support.v4.widget.NestedScrollView，其中就包含“.”;
					if (-1 == name.indexOf('.')) {
    ...
    void rInflate(XmlPullParser parser, View parent, Context context,
        ...
        final int depth = parser.getDepth();//获取视图树深度
        int type;
        //逐一解析元素
        while (((type = parser.next()) != XmlPullParser.END_TAG ||
            ...
            if (TAG_REQUEST_FOCUS.equals(name)) {//设置焦点
                parseRequestFocus(parser, parent);
            } else if (TAG_TAG.equals(name)) {//添加tag
                parseViewTag(parser, parent, attrs);
            } else if (TAG_INCLUDE.equals(name)) {//解析<include/>标签
                if (parser.getDepth() == 0) {//<include/>不能为根视图
                    throw new InflateException("<include /> cannot be the root element");
                }
                parseInclude(parser, context, parent, attrs);
            } else if (TAG_MERGE.equals(name)) {//<merge />必需为根视图
