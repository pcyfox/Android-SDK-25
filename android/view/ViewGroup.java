 ...
 * <a href="{@docRoot}guide/topics/ui/declaring-layout.html">XML Layouts</a>
 * developer guide.
 * </p>
 * </div>
 *
 ...
 * simple {@link android.widget.FrameLayout} along with the ability to stack
 ...
 *
 * <p>
 * If you are implementing XML layout attributes as shown in the example, this
 * is the corresponding definition for them that would go in
 * <code>res/values/attrs.xml</code>:
 * </p>
 *
...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
	...
	/**
	 * Views which have been hidden or removed which need to be animated on their
	 * way out. This field should be made private, so it is hidden from the SDK.
	 * {@hide}
	 */
	...
	// Current frontmost child that can accept drag and lies under the drag
	// location.
	...
	// 由于存在多点触控，链表设计能很好记录触摸的目标，“触摸目标”是指在ACTION_DOWN时检测到能处理该事件的子View的相关信息
	...
	private TouchTarget mFirstTouchTarget;// "触摸目标"链表的头部
	...
	@ViewDebug.ExportedProperty(flagMapping = {
			@ViewDebug.FlagToString(mask = FLAG_CLIP_CHILDREN, equals = FLAG_CLIP_CHILDREN, name = "CLIP_CHILDREN"),
			@ViewDebug.FlagToString(mask = FLAG_CLIP_TO_PADDING, equals = FLAG_CLIP_TO_PADDING, name = "CLIP_TO_PADDING"),
			@ViewDebug.FlagToString(mask = FLAG_PADDING_NOT_NULL, equals = FLAG_PADDING_NOT_NULL, name = "PADDING_NOT_NULL") }, formatToHexString = true)
	protected int mGroupFlags;

	/**
	 * Either {@link #LAYOUT_MODE_CLIP_BOUNDS} or
	...
	// When set, dispatchDraw() will invoke invalidate(); this is set by drawChild()
	// when
	...
	// When set, this ViewGroup converts calls to invalidate(Rect) to invalidate()
	// during a
	...
	// This flag is set when the layout animation is over (after FLAG_ANIMATION_DONE
	// is set)
	private static final int FLAG_NOTIFY_ANIMATION_LISTENER = 0x200;

	/**
	 * When set, the drawing method will call
	 * {@link #getChildDrawingOrder(int, int)} to get the index of the child to draw
	 * for that iteration.
	 *
	 * @hide
	 */
	...
	/**
	 * When set, this ViewGroup will split MotionEvents to multiple child Views when
	 * appropriate.
	 */
	 ...
	 * children when adding new views. This is used to prevent multiple onAttached
	 * calls when a ViewGroup adds children in its own onAttached method.
	 */
	private static final int FLAG_PREVENT_DISPATCH_ATTACHED_TO_WINDOW = 0x400000;

	/**
	 * When true, indicates that a layoutMode has been explicitly set, either with
	 * an explicit call to {@link #setLayoutMode(int)} in code or from an XML
	 * resource. This distinguishes the situation in which a layout mode was
	 * inherited from one of the ViewGroup's ancestors and cached locally.
	 */
	...
	/**
	 * When set, focus will not be permitted to enter this group if a touchscreen is
	 * present.
	 */
	static final int FLAG_TOUCHSCREEN_BLOCKS_FOCUS = 0x4000000;

	/**
	 * When true, indicates that a call to startActionModeForChild was made with the
	 * type parameter and should not be ignored. This helps in backwards
	 * compatibility with the existing method without a type.
	 *
	...
	/**
	 * When true, indicates that a call to startActionModeForChild was made without
	 * the type parameter. This helps in backwards compatibility with the existing
	 * method without a type.
	 *
	 * @see #startActionModeForChild(View, android.view.ActionMode.Callback)
	 * @see #startActionModeForChild(View, android.view.ActionMode.Callback, int)
	 */
	...
	/**
	 * When set, indicates that a call to showContextMenuForChild was made with
	 * explicit coordinates within the initiating child view.
	 */
	 ...
	 * the raw values of {@link #getLeft() left}, {@link #getTop() top},
	 ...
	 * need to cover a larger area to allow other effects, such as shadows and
	 * glows, to be drawn.
	 */
	...
	/**
	 * We clip to padding when FLAG_CLIP_TO_PADDING and FLAG_PADDING_NOT_NULL are
	...
	// Number of valid children in the mChildren array, the rest should be null or
	// not
	...
	// The set of views that are currently being transitioned. This list is used to
	// track views
	 ...
	 * {@link #SCROLL_AXIS_HORIZONTAL} {@link #SCROLL_AXIS_VERTICAL} or
	 * {@link #SCROLL_AXIS_NONE} for null.
	 */
	...
	/**
	 * Empty ActionMode used as a sentinel in recursive entries to
	 ...
	 *         {@link #FOCUS_AFTER_DESCENDANTS}, {@link #FOCUS_BLOCK_DESCENDANTS}.
	 */
	 ...
	 * view to take focus in {@link #requestFocus(int, android.graphics.Rect)}.
	 *
	 * @param focusability one of {@link #FOCUS_BEFORE_DESCENDANTS},
	...
	public void setDescendantFocusability(int focusability) {
		...
		default:
			throw new IllegalArgumentException(
					"must be one of FOCUS_BEFORE_DESCENDANTS, " + "FOCUS_AFTER_DESCENDANTS, FOCUS_BLOCK_DESCENDANTS");
		}
	 ...
	 * @param direction One of FOCUS_UP, FOCUS_DOWN, FOCUS_LEFT, and FOCUS_RIGHT, or
	 ...
	 * <p>
	 * If an {@link android.view.View.AccessibilityDelegate} has been specified via
	 * calling
	...
	/**
	 * Set whether this ViewGroup should ignore focus requests for itself and its
	 * children. If this option is enabled and the ViewGroup or a descendant
	 * currently has focus, focus will proceed forward.
	 *
	 * @param touchscreenBlocksFocus true to enable blocking focus in the presence
	 *                               of a touchscreen
	 */
	...
	/**
	 * Check whether this ViewGroup should ignore focus requests for itself and its
	 * children.
	 */
	 ...
	 * @param oldVisibility The previous visibility value (GONE, INVISIBLE, or
	 *                      VISIBLE).
	...
	boolean dispatchVisibilityAggregated(boolean isVisible) {
		...
		final View[] children = mChildren;
		for (int i = 0; i < count; i++) {
			// Only dispatch to visible children. Not visible children and their subtrees
			// already
	...
	boolean dispatchDragEnterExitInPreN(DragEvent event) {
		if (event.mAction == DragEvent.ACTION_DRAG_EXITED && mCurrentDragChild != null) {
			// The drag exited a sub-tree of views; notify of the exit all descendants that
			// are in
			// entered state.
			// We don't need this recursive delivery for ENTERED events because they get
			// generated
			// from the recursive delivery of LOCATION/DROP events, and hence, don't need
			// their own
			// recursion.
	...
	public boolean dispatchDragEvent(DragEvent event) {
		...
		case DragEvent.ACTION_DRAG_STARTED: {
			...
			if (!retval) {
				// Neither us nor any of our children are interested in this drag, so stop
				// tracking
	...
	public PointerIcon onResolvePointerIcon(MotionEvent event, int pointerIndex) {
		...
		// The pointer is not a child or the child has no preferences, returning the
		// default
		// implementation.
	...
	protected boolean dispatchHoverEvent(MotionEvent event) {
		...
		// Send events to the hovered children and build a new list of hover targets
		// until
		...
		// Send exit events to all previously hovered children that are no longer
		// hovered.
	 ...
	 * group or to the view group's own {@link #onHoverEvent} to allow the view
	 ...
	 * implementing this method and returning <code>true</code> to indicate that it
	 * would like to intercept hover events. The view group must continuously return
	 * <code>true</code> from {@link #onInterceptHoverEvent} for as long as it
	 * wishes to continue intercepting hover events from its children.
	 * </p>
	 * <p>
	 * Interception preserves the invariant that at most one view can be hovered at
	 * a time by transferring hover focus from the currently hovered child to the
	 * view group or vice-versa as needed.
	 * </p>
	 * <p>
	 * If this method returns <code>true</code> and a child is already hovered, then
	 * the child view will first receive a hover exit event and then the view group
	 * itself will receive a hover enter event in {@link #onHoverEvent}. Likewise,
	 ...
	 * @return True if the view group would like to intercept the hover event and
	 *         prevent its children from receiving it.
	 */
    ...
    //关于多点触控：
	// 点触控模式下：当有一个手指触摸到View后不松手，屏幕中又有一个手指手指按下，然后两指松开（不分顺序）
	// 当新按下的点找不到消费该事件的View（另一只手指触摸空白区域）或与ACTION_DOWN时触摸的是同一个View时
	// 收到MotionEvent的事件序列为：
	// action=ACTION_DOWN
	// action=ACTION_POINTER_DOWN
	// action=ACTION_POINTER_UP
	// action=ACTION_UP
	// 当新按下的手指能找到消费该事件的View时(另一只手指触摸到一个Button)：
	// action=ACTION_DOWN
	// action=ACTION_MOVE
	// action=ACTION_UP
	// 再多的手指触摸同一个View，始终只有一个ACTION_DOWN、ACTION_UP，
	// 而ACTION_POINTER_DOWN、与ACTION_POINTER_UP的数量=总的触摸点-1
	// 因此多个点触摸同一个View时只会发生一次点击行为
	// 这是怎么实现的呢？
	@Override
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (onFilterTouchEventForSecurity(ev)) {
			...
			// 一旦onInterceptTouchEvent（）返回true，使得intercepted = true后，
			// 后续的MotionEvent.ACTION_MOVE及MotionEvent.ACTION_UP事件中将无法改变intercepted的值.因为只有
			// actionMasked == MotionEvent.ACTION_DOWN时才会执行intercepted =
			// onInterceptTouchEvent(ev)从而更新intercepted。
			// 意味着子元素将没机会处理后续的事件，所以在使用外部拦截法时千万不能在MotionEvent.ACTION_DOWN中拦截事件
			// Check for interception.
			final boolean intercepted;// 自己是否需要拦截该事件
			// 由此可以得知：
			// 1、MotionEvent.ACTION_DOWN时一定会去调用onInterceptTouchEvent（），以及检查FLAG_DISALLOW_INTERCEPT标识
			// 2、其它后续事件，只有存在“触摸目标”mFirstTouchTarget时才会调用onInterceptTouchEvent（），以及检查FLAG_DISALLOW_INTERCEPT标识
			if (actionMasked == MotionEvent.ACTION_DOWN || mFirstTouchTarget != null) {// 事件为ACTION_DOWN，或者有touchTarget
				final boolean disallowIntercept = (mGroupFlags & FLAG_DISALLOW_INTERCEPT) != 0;
				/**
				 * 此处判断当前父控件是否不干预拦截事件，默认为false， 会调用onInterceptTouchEvent(ev)判断是否拦截事件。
				 * 一旦disallowIntercept等于true，子View就成功干预了ViewGroup的拦截行为。
				 * ViewGroup不会调用onInterceptTouchEvent(ev)判断是否拦截事件。
				 *
				 * 子View可以通过requestDisallowInterceptTouchEvent()方法设置mGroupFlags。
				 *
				 * onInterceptTouchEvent(ev)默认返回false不拦截事件， 继承自ViewGroup的子类可以重写该方法决定是否拦截事件，
				 * 如果子控件调用了父控件的requestDisallowInterceptTouchEvent()方法，
				 * 父控件就不会调用onInterceptTouchEvent(ev)方法， 此时的onInterceptTouchEvent(ev)是无效的。
				 *
				 * 如果拦截事件，intercepted会置为true， 否则不拦截事件，intercepted会置为false。
				 */
				...
				// “触摸目标”意味着在ACTION_DOWN时无消费事件的子View
				// 后续事件将被拦截，不再响应
			...
			// 通常发生在鼠标右键点击时，以及View飞出了可滑动的布局（如在ListView、ScrollerView中）时
			final boolean canceled = resetCancelNextUpFlag(this) || actionMasked == MotionEvent.ACTION_CANCEL;

			// Update list of touch targets for pointer down, if needed.
			final boolean split = (mGroupFlags & FLAG_SPLIT_MOTION_EVENTS) != 0;// 是否支持多点触控，默认为true
			// 当事件已经做出派分时，记录派分对应的控件。
			TouchTarget newTouchTarget = null;
			// 用于过滤已派分的事件，避免事件重复派分
			boolean alreadyDispatchedToNewTouchTarget = false;
			// 只有事件未被取消并且未被拦截，才会去执行分发逻辑
			if (!canceled && !intercepted) {
				...
				View childWithAccessibilityFocus = ev.isTargetAccessibilityFocus() ? findChildWithAccessibilityFocus()
						: null;
				if (actionMasked == MotionEvent.ACTION_DOWN
						|| (split && actionMasked == MotionEvent.ACTION_POINTER_DOWN/*
																					 * ACTION_DOWN后未执行ACTION_UP时：多点触控模式下
																					 * ，有其它手指按下时触发该事件
																					 */)
						|| actionMasked == MotionEvent.ACTION_HOVER_MOVE/* 鼠标悬停在View上 */) {
					// 获取手指序号
					final int actionIndex = ev.getActionIndex(); // always 0 for down
					// ev.getPointerId(actionIndex)获取手指id
					...
					if (newTouchTarget == null && childrenCount != 0) {
						...
						for (int i = childrenCount - 1; i >= 0; i--) {
							...
							// canViewReceivePointerEvents：以可见性或者有动画返回true
							// isTransformedTouchPointInView：以触发点没在child的边界内作为判断依据
							if (!canViewReceivePointerEvents(child)
									|| !isTransformedTouchPointInView(x, y, child, null)) {
								ev.setTargetAccessibilityFocus(false);
								continue;
							}
							// 判断该child是否已经包含在mFirstTouchTarget，如果是则更新pointerIdBits并退出循环
							// 场景：多指触控，多个触摸点在同一个View时，收到MotionEvent.ACTION_POINTER_DOWN，此时newTouchTarget不为null。循环将被break
							...
							// 在dispatchTransformedTouchEvent方法中开始分发事件
							if (dispatchTransformedTouchEvent(ev, false, child, idBitsToAssign)) {
								...
								// 生成一个新的TouchTarget（包裹着消化事件的View），
								// 并添加到mFirstTouchTarget头部，并使newTouchTarget指向生成的TouchTarget
					...
					// 即多点触控中，无法找到消耗ACTION_POINTER_DOWN事件的View，ACTION_POINTER_DOWN无法找到目标时视为ACTION_DOWN目标接收派分
	 ...
	 *
	 * This is called within a tight loop, so you are not allowed to allocate
	 * objects, including the return array. Instead, you should return a
	 * pre-allocated list that will be cleared after the dispatch is finished.
	 * 
	 * @hide
	 */
	...
	/**
	 * Adds a touch target for specified child to the beginning of the list. Assumes
	 * the target child is not already present.
	 */
	private TouchTarget addTouchTarget(@NonNull View child, int pointerIdBits) {
		final TouchTarget target = TouchTarget.obtain(child, pointerIdBits);
		target.next = mFirstTouchTarget;
		mFirstTouchTarget = target;// 将mFirstTouchTarget置为头节点
		return target;
	}
	 ...
	 * into its coordinate space. Child must not be null.
	 * 
	 * @hide
	 */
	...
	private boolean dispatchTransformedTouchEvent(MotionEvent event, boolean cancel, View child,
		...
		if (cancel || oldAction == MotionEvent.ACTION_CANCEL) {
			...
			if (child == null) {// 说明没有找到合适的子view来处理该事件
				// 调用View中的dispatchTouchEvent方法，即调用View类中的dispatchTouchEvent方法
				// View类中的这个方法将决定是否消费掉该事件
		...
		// If for some reason we ended up in an inconsistent（不一致的） state where it looks
		// like we
		...
		// If the number of pointers is the same and we don't need to perform any
		// fancy（昂贵的）
		// irreversible（不可逆的; 不能翻转的; 不能倒置的） transformations, then we can reuse the
		// motion event for this
	 ...
	 * touch event dispatch. This behavior is enabled by default for applications
	 * that target an SDK version of {@link Build.VERSION_CODES#HONEYCOMB} or newer.
	 *
	 * <p>
	 * When this option is enabled MotionEvents may be split and dispatched to
	 * different child views depending on where each pointer initially went down.
	 * This allows for user interactions such as scrolling two panes of content
	 * independently, chording of buttons, and performing independent gestures on
	 * different pieces of content.
	 *
	 * @param split <code>true</code> to allow MotionEvents to be split and
	...
	public void setMotionEventSplittingEnabled(boolean split) {
		...
		// but perhaps this should handle that case and send ACTION_CANCELs to any child
		// views
	...
	/**
	 * Returns true if this ViewGroup should be considered as a single entity for
	 * removal when executing an Activity transition. If this is false, child
	 * elements will move individually during the transition.
	 *
	 * @return True if the ViewGroup should be acted on together during an Activity
	 *         transition. The default value is true when there is a non-null
	 *         background or if {@link #getTransitionName()} is not null or if a
	...
	/**
	 * Changes whether or not this ViewGroup should be treated as a single entity
	 * during Activity Transitions.
	 * 
	 * @param isTransitionGroup Whether or not the ViewGroup should be treated as a
	 *                          unit in Activity transitions. If false, the
	 *                          ViewGroup won't transition, only its children. If
	 *                          true, the entire ViewGroup will transition together.
	 ...
	 * allows you to watch events as they are dispatched to your children, and take
	 * ownership of the current gesture at any point.
	 *
	 * <p>
	 ...
	 * method as well as this one in the correct way. Events will be received in the
	 * following order:
	 *
	 * <ol>
	 * <li>You will receive the down event here.
	 * <li>The down event will be handled either by a child of this view group, or
	 * given to your own onTouchEvent() method to handle; this means you should
	 * implement onTouchEvent() to return true, so you will continue to see the rest
	 * of the gesture (instead of looking for a parent view to handle it). Also, by
	 * returning true from onTouchEvent(), you will not receive any following events
	 * in onInterceptTouchEvent() and all touch processing must happen in
	 * onTouchEvent() like normal.
	 * <li>For as long as you return false from this function, each following event
	 * (up to and including the final up) will be delivered first here and then to
	 * the target's onTouchEvent().
	 * <li>If you return true from here, you will not receive any following events:
	 * the target view will receive the same event but with the action
	 * {@link MotionEvent#ACTION_CANCEL}, and all further events will be delivered
	 * to your onTouchEvent() method and no longer appear here.
	 * </ol>
	 *
	 * @param ev The motion event being dispatched down the hierarchy.
	 * @return Return true to steal motion events from the children and have them
	 *         dispatched to this ViewGroup through onTouchEvent(). The current
	 *         target will receive an ACTION_CANCEL event, and no further messages
	 ...
	 *
	 * Uses {@link #onRequestFocusInDescendants(int, android.graphics.Rect)} to find
	 ...
	 * {@link ViewGroup#requestFocus(int, android.graphics.Rect)} when it wants to
	 * request focus within its children. Override this to customize how your
	 * {@link ViewGroup} requests focus within its children.
	 * 
	 * @param direction             One of FOCUS_UP, FOCUS_DOWN, FOCUS_LEFT, and
	 *                              FOCUS_RIGHT
	 * @param previouslyFocusedRect The rectangle (in this View's coordinate system)
	 *                              to give a finer grained hint about where focus
	 *                              is coming from. May be null if there is no hint.
	...
	/**
	 * Dispatch creation of {@link ViewStructure} down the hierarchy. This
	 * implementation adds in all child views of the view group, in addition to
	 * calling the default View implementation.
	 */
	@Override
					...
					for (int i = 0; i < childrenCount; i++) {
						...
						try {
							...
							if (mContext.getApplicationInfo().targetSdkVersion < Build.VERSION_CODES.M) {
								Log.w(TAG, "Bad getChildDrawingOrder while collecting assist @ " + i + " of "
										+ childrenCount, e);
	...
	/**
	 * Counts the number of children of this View that will be sent to an
	 * accessibility service.
	 *
	 * @return The number of children an {@code AccessibilityNodeInfo} rooted at
	 *         this View would have.
	 */
	 ...
	 * <code>super.onNestedPrePerformAccessibilityAction</code>
	 * </p>
	 *
	 ...
	 * @return false by default. Subclasses should return true if they handle the
	 *         event.
	 */
	@Override
	 ...
	 * {@link #saveHierarchyState(android.util.SparseArray)} freeze()} to only this
	...
	/**
	 * Perform dispatching of a
	 * {@link #restoreHierarchyState(android.util.SparseArray)} to only this view,
	...
	private static void drawRectCorners(Canvas canvas, int x1, int y1, int x2, int y2, Paint paint, int lineLength,
			int lineWidth) {
	...
	private static void fillDifference(Canvas canvas, int x2, int y2, int x3, int y3, int dx1, int dy1, int dx2,
			int dy2, Paint paint) {
		int x1 = x2 - dx1;
		int y1 = y2 - dy1;
	...
	protected void onDebugDraw(Canvas canvas) {
		...
		// Draw optical bounds
		{
			...
			for (int i = 0; i < getChildCount(); i++) {
				...
				if (c.getVisibility() != View.GONE) {
					...
					drawRect(canvas, paint, c.getLeft() + insets.left, c.getTop() + insets.top,
		...
		// Draw clip bounds
		{
			...
			for (int i = 0; i < getChildCount(); i++) {
				View c = getChildAt(i);
				if (c.getVisibility() != View.GONE) {
					drawRectCorners(canvas, c.getLeft(), c.getTop(), c.getRight(), c.getBottom(), paint, lineLength,
							lineWidth);
				}
			}
		}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (clipToPadding) {
			...
			canvas.clipRect(mScrollX + mPaddingLeft, mScrollY + mPaddingTop, mScrollX + mRight - mLeft - mPaddingRight,
		...
		// Only use the preordered list if not HW accelerated, since the HW pipeline
		// will do the
	 ...
	 * yet exist. In addition to {@link ViewOverlay}'s support for drawables,
	 ...
	 *
	 * <p>
	 * Note: Overlays do not currently work correctly with {@link SurfaceView} or
	 * {@link TextureView}; contents in overlays for these types of views may not
	 * display correctly.
	 * </p>
	 *
	...
	/**
	 * Returns the index of the child to draw for this iteration. Override this if
	 * you want to change the drawing order of children. By default, it returns i.
	 * <p>
	...
	/**
	 * Populates (and returns) mPreSortedChildren with a pre-ordered list of the
	 * View's children, sorted first by Z, then by child drawing order (if
	 * applicable). This list must be cleared after use to avoid leaking child
	 * Views.
	 *
	 * Uses a stable, insertion sort which is commonly O(n) for ViewGroups with very
	 * few elevated children.
	 */
	 ...
	 * parent ViewGroup does not need to recreate its own display list, which would
	 * happen if it went through the normal draw/dispatchDraw mechanisms.
	 *
	 * @hide
	 */
	@Override
	 ...
	 * child's scrolled origin is at 0, 0, and applying any animation
	 * transformations.
	 *
	...
	/**
	 * Returns whether this group's children are clipped to their bounds before
	 * drawing. The default value is true.
	 * 
	 * @see #setClipChildren(boolean)
	 *
	 * @return True if the group's children will be clipped to their bounds, false
	 *         otherwise.
	 */
	 ...
	 * <p>
	 * By default, children are clipped to the padding of their parent ViewGroup.
	 * This clipping behavior is only enabled if padding is non-zero.
	 *
	 * @param clipToPadding true to clip children to the padding of the group, and
	 *                      resize (but not clip) any EdgeEffect to the padded
	...
	/**
	 * Returns whether this ViewGroup will clip its children to its padding, and
	 * resize (but not clip) any EdgeEffect to the padded region, if padding is
	 * present.
	 * <p>
	 * By default, children are clipped to the padding of their parent Viewgroup.
	 * This clipping behavior is only enabled if padding is non-zero.
	 *
	 * @return true if this ViewGroup clips children to its padding and resizes (but
	...
	/**
	 * Dispatches drawable hotspot changes to child views that meet at least one of
	 ...
	 *
	 * @param enabled True to enable static transformations on children, false
	 *                otherwise.
	 *
	...
	/**
	 * This method adds a view to this container at the specified index purely for
	 * the purposes of allowing that view to draw even though it is not a normal
	 * child of the container. That is, the view does not participate in layout,
	 * focus, accessibility, input, or other normal view operations; it is purely an
	 * item to be drawn during the normal rendering operation of this container. The
	 * index that it is added at is the order in which it will be drawn, with
	 * respect to the other views in the container. For example, a transient view
	 * added at index 0 will be drawn before all other views in the container
	 * because it will be drawn first (including before any real view at index 0).
	 * There can be more than one transient view at any particular index; these
	 * views will be drawn in the order in which they were added to the list of
	 * transient views. The index of transient views can also be greater than the
	 * number of normal views in the container; that just means that they will be
	 * drawn after all other views are drawn.
	 *
	 * <p>
	 * Note that since transient views do not participate in layout, they must be
	 * sized manually or, more typically, they should just use the size that they
	 * had before they were removed from their container.
	 * </p>
	 *
	 * <p>
	 * Transient views are useful for handling animations of views that have been
	 * removed from the container, but which should be animated out after the
	 * removal. Adding these views as transient views allows them to participate in
	 * drawing without side-effecting the layout of the container.
	 * </p>
	 *
	 * <p>
	 * Transient views must always be explicitly {@link #removeTransientView(View)
	 * removed} from the container when they are no longer needed. For example, a
	 * transient view which is added in order to fade it out in its old location
	 * should be removed once the animation is complete.
	 * </p>
	 *
	 * @param view  The view to be added
	 * @param index The index at which this view should be drawn, must be >= 0. This
	 *              value is relative to the {@link #getChildAt(int) index} values
	 *              in the normal child list of this container, where any transient
	 *              view at a particular index will be drawn before any normal child
	 *              at that same index.
	 *
	 * @hide
	 */
	...
	/**
	 * Removes a view from the list of transient views in this container. If there
	 * is no such transient view, this method does nothing.
	 *
	...
	/**
	 * Given a valid position within the list of transient views, returns the index
	 * of the transient view at that position.
	 *
	 * @param position The position of the index being queried. Must be at least 0
	 *                 and less than the value returned by
	 *                 {@link #getTransientViewCount()}.
	 ...
	 *                 and less than the value returned by
	 *                 {@link #getTransientViewCount()}.
	 * @return The transient view stored in the given position if the position is
	...
	/**
	 * Register a callback to be invoked when a child is added to or removed from
	 * this view.
	 *
	...
	/**
	 * Called when a child view is removed from this ViewGroup. Overrides should
	 * always call super.onViewRemoved.
	 *
	 ...
	 *
	 * @param child                the view to add to the group
	 * @param index                the index at which the child must be added or -1
	 *                             to add last
	 * @param params               the layout parameters to associate with the child
	 ...
	 * animations defined in that LayoutTransition object. By default, the
	 * transition object is null (so layout changes are not animated).
	 *
	 * <p>
	 * Replacing a non-null transition will cause that previous transition to be
	 * canceled, if it is currently running, to restore this container to its
	 * correct post-transition state.
	 * </p>
	 *
	 * @param transition The LayoutTransition object that will animated changes in
	 *                   layout. A value of <code>null</code> means no transition
	 *                   will run on layout changes.
	 * @attr ref android.R.styleable#ViewGroup_animateLayoutChanges
	 */
	 ...
	 *         changes in layout. A value of <code>null</code> means no transition
	 *         will run on layout changes.
	 */
	 ...
	 * views it will render. An example is a Gallery or a ListView, which may "have"
	 * 50 children, but actually only render the number of children that can
	 * currently fit inside the object on screen. Do not call this method unless you
	 * are extending ViewGroup and understand the view measuring and layout
	 * pipeline.
	 *
	 * <p>
	 * <strong>Note:</strong> do not invoke this method from
	 ...
	 * include also making any appropriate {@link #requestLayout()} or
	 * {@link #invalidate()} calls. For example, callers can {@link #post(Runnable)
	 * post} a {@link Runnable} which performs a {@link #requestLayout()} on the
	 * next frame, after all detach/remove calls are finished, causing layout to be
	 * run prior to redrawing the view hierarchy.
	 *
	 * @param child   the child to be definitely removed from the view hierarchy
	 * @param animate if true and the view has an animation, the view is placed in
	...
	/**
	 * Attaches a view to this view group. Attaching a view assigns this group as
	 * the parent, sets the layout parameters and puts the view in the list of
	 * children so that it can be retrieved by calling {@link #getChildAt(int)}.
	 * <p>
	 * This method is intended to be lightweight and makes no assumptions about
	 * whether the parent or child should be redrawn. Proper use of this method will
	 * include also making any appropriate {@link #requestLayout()} or
	 * {@link #invalidate()} calls. For example, callers can {@link #post(Runnable)
	 * post} a {@link Runnable} which performs a {@link #requestLayout()} on the
	 * next frame, after all detach/attach calls are finished, causing layout to be
	 * run prior to redrawing the view hierarchy.
	 * <p>
	...
	protected void attachViewToParent(View child, int index, LayoutParams params) {
		...
		child.mParent = this;
		child.mPrivateFlags = (child.mPrivateFlags & ~PFLAG_DIRTY_MASK & ~PFLAG_DRAWING_CACHE_VALID) | PFLAG_DRAWN
				| PFLAG_INVALIDATED;
	...
	/**
	 * Detaches a view from its parent. Detaching a view should be followed either
	 * by a call to
	 * {@link #attachViewToParent(View, int, android.view.ViewGroup.LayoutParams)}
	 * or a call to {@link #removeDetachedView(View, boolean)}. Detachment should
	 * only be temporary; reattachment or removal should happen within the same
	 * drawing cycle as detachment. When a view is detached, its parent is null and
	 * cannot be retrieved by a call to {@link #getChildAt(int)}.
	 *
	 ...
	 * this ViewGroup is already fully invalidated or if the dirty rectangle does
	 * not intersect with this ViewGroup's bounds.
	 */
	@Override
	...
	/**
	 * Native-calculated damage path Returns false if this path was unable to
	 * complete successfully. This means it hit a ViewParent it doesn't recognize
	 * and needs to fall back to calculating damage area
	 * 
	 * @hide
	 */
	...
	/**
	 * @param forceParentCheck true to guarantee that this call will propagate to
	 *                         all ancestors, false otherwise
	 *
	 * @hide
	 */
	public boolean getChildVisibleRect(View child, Rect r, android.graphics.Point offset, boolean forceParentCheck) {
		// It doesn't make a whole lot of sense to call this on a view that isn't
		// attached,
	...
	/**
	 * Runs the layout animation. Calling this method triggers a relayout of this
	 * view group.
	 */
	 ...
	 * this view group. This can be used to restart the layout animation when the
	 * content of the view group changes or when the activity is paused and resumed.
	 */
	 ...
	 *
	 * @deprecated As of {@link android.os.Build.VERSION_CODES#M}, this property is
	 *             ignored. Caching behavior of children may be controlled through
	 ...
	 * By default, the drawing cache is enabled but this will prevent nested layout
	 ...
	 *
	 * @deprecated As of {@link android.os.Build.VERSION_CODES#M}, this property is
	 *             ignored. Child views may no longer have their caching behavior
	 *             disabled by parents.
	 */
	...
	/**
	 * Indicates whether this ViewGroup will always try to draw its children using
	 * their drawing cache. This property can be set to true when the cache
	 * rendering is slightly different from the children's normal rendering.
	 * Renderings can be different, for instance, when the cache's quality is set to
	 * low.
	 *
	 * When this property is disabled, the ViewGroup will use the drawing cache of
	 * its children only when asked to. It's usually the task of subclasses to tell
	 ...
	 *
	 * @deprecated As of {@link android.os.Build.VERSION_CODES#M}, this property is
	 *             ignored. Child views may no longer be forced to cache their
	 *             rendering state by their parents. Use
	 *             {@link View#setLayerType(int, Paint)} on individual Views
	 *             instead.
	 */
	 ...
	 * property is ignored when {@link #isAlwaysDrawnWithCacheEnabled()} is true. A
	 * child's drawing cache will be used only if it has been enabled.
	 *
	 ...
	 * animating.
	 *
	 * @param enabled true if children should be drawn with their cache, false
	 *                otherwise
	 *
	 * @see #setAlwaysDrawnWithCacheEnabled(boolean)
	...
	/**
	 * Indicates whether the ViewGroup is drawing its children in the order defined
	 * by {@link #getChildDrawingOrder(int, int)}.
	 *
	 ...
	 * method {@link #getChildDrawingOrder(int, int)}.
	 * <p>
	 * Note that {@link View#getZ() Z} reordering, done by
	 * {@link #dispatchDraw(Canvas)}, will override custom child ordering done via
	 * this method.
	 *
	 * @param enabled true if the order of the children when drawing is determined
	 ...
	 * @see #setAnimationCacheEnabled(boolean)
	 *
	 * @param drawingCacheToKeep one or a combination of
	 *                           {@link #PERSISTENT_NO_CACHE},
	 *                           {@link #PERSISTENT_ANIMATION_CACHE},
	 *                           {@link #PERSISTENT_SCROLLING_CACHE} and
	 *                           {@link #PERSISTENT_ALL_CACHES}
	 */
	...
	public int getLayoutMode() {
		if (mLayoutMode == LAYOUT_MODE_UNDEFINED) {
			int inheritedLayoutMode = (mParent instanceof ViewGroup) ? ((ViewGroup) mParent).getLayoutMode()
					: LAYOUT_MODE_DEFAULT;
	 ...
	 * are either {@link #LAYOUT_MODE_CLIP_BOUNDS} or
	 * {@link #LAYOUT_MODE_OPTICAL_BOUNDS}.
	 *
	 ...
	 *
	 * @return an instance of {@link android.view.ViewGroup.LayoutParams} or one of
	 *         its descendants
	 */
	 ...
	 *
	 * @param p The layout parameters to convert into a suitable set of layout
	...
	/**
	 * ViewGroupb并未实现onMeasure（）方法， Ask all of the children of this view to measure
	 * themselves, taking into account both the MeasureSpec requirements for this
	 * view and its padding. We skip children that are in the GONE state The heavy
	 ...
	 * @param child                   The child to measure
	 * @param parentWidthMeasureSpec  The width requirements for this view
	 * @param widthUsed               Extra space that has been used up by the
	 *                                parent horizontally (possibly by other
	 *                                children of the parent)
	 * @param parentHeightMeasureSpec The height requirements for this view
	 * @param heightUsed              Extra space that has been used up by the
	 *                                parent vertically (possibly by other children
	 *                                of the parent)
	 */
	...
	/**
	 * Does the hard part of measureChildren: figuring out the MeasureSpec to pass
	 * to a particular child. This method figures out the right MeasureSpec for one
	...
	/**
	 * Cleanup a view when its animation is done. This may mean removing it from the
	 * list of disappearing views.
	 *
	...
	/**
	 * Utility function called by View during invalidation to determine whether a
	 * view that is invisible or gone should still be invalidated because it is
	 * being transitioned (and therefore still needs to be drawn).
	 */
	 ...
	 * allows animations, such as those used by {@link android.app.Fragment} and
	 * {@link android.animation.LayoutTransition} to animate the removal of views. A
	 * call to this method should always be accompanied by a later call to
	 * {@link #endViewTransition(View)}, such as after an animation on the View has
	 * finished, so that the View finally gets removed.
	 *
	 * @param view The View object to be kept visible even if it gets removed from
	 *             its parent.
	 */
	 ...
	 * {@link #startViewTransition(View)}. The given View is finally removed from
	 * its parent and will no longer be displayed. Note that this method does not
	 * perform the functionality of removing a view from its parent; it just
	 * discontinues the display of a View that has previously been removed.
	 *
	 * @return view The View object that has been removed but is being kept around
	...
	/**
	 * Tells this ViewGroup to suppress all layout() calls until layout suppression
	 * is disabled with a later call to suppressLayout(false). When layout
	 * suppression is disabled, a requestLayout() call is sent if layout() was
	 * attempted while layout was being suppressed.
	 *
	 * @hide
	 */
	...
	public boolean gatherTransparentRegion(Region region) {
		...
		// Instead of naively traversing the view tree, we have to traverse according to
		// the Z
		// order here. We need to go with the same order as dispatchDraw().
		// One example is that after surfaceView punch a hole, we will still allow other
		// views drawn
		// on top of that hole. In this case, those other views should be able to cut
		// the
		...
		if ((mGroupFlags & FLAG_NOTIFY_CHILDREN_ON_DRAWABLE_STATE_CHANGE) != 0) {
			if ((mGroupFlags & FLAG_ADD_STATES_FROM_CHILDREN) != 0) {
				throw new IllegalStateException(
						"addStateFromChildren cannot be enabled if a" + " child has duplicateParentState set to true");
			}
	 ...
	 * animations that need to start after the layout/setup phase. The request is
	 * forwarded to the ViewAncestor, who starts all pending transitions prior to
	 * the drawing phase in the current traversal.
	 *
	...
	public boolean resolveRtlPropertiesIfNeeded() {
		...
		// We dont need to resolve the children RTL properties if nothing has changed
		// for the parent
		if (result) {
	 ...
	 * descendants of this ViewGroup. Generally, this should be done for containers
	 * that can scroll, such as a List. This prevents the pressed state from
	 * appearing when the user is actually trying to scroll the content.
	 *
	 ...
	 *
	 *             <p>
	 *             The default implementation of onStopNestedScroll calls
	 *             {@link #stopNestedScroll()} to halt any recursive nested
	 ...
	 *
	 * <p>
	 * A ViewGroup returning something other than {@link #SCROLL_AXIS_NONE} is
	 * currently acting as a nested scrolling parent for one or more descendant
	 ...
	 * out. See {@link android.R.styleable#ViewGroup_Layout ViewGroup Layout
	 ...
	 * <ul>
	 * <li>FILL_PARENT (renamed MATCH_PARENT in API Level 8 and higher), which means
	...
	public static class LayoutParams {
		 ...
		 * that the view wants to be as big as its parent, minus the parent's padding,
		 * if any. This value is deprecated starting in API Level 8 and replaced by
		 * {@link #MATCH_PARENT}.
		 */
		 ...
		 * that the view wants to be as big as its parent, minus the parent's padding,
		 * if any. Introduced in API Level 8.
		 */
		 ...
		 *
		 * @param width  the width, either {@link #WRAP_CONTENT}, {@link #FILL_PARENT}
		 *               (replaced by {@link #MATCH_PARENT} in API Level 8), or a fixed
		 *               size in pixels
		 * @param height the height, either {@link #WRAP_CONTENT}, {@link #FILL_PARENT}
		 *               (replaced by {@link #MATCH_PARENT} in API Level 8), or a fixed
		 *               size in pixels
		 */
	...
	public static class MarginLayoutParams extends ViewGroup.LayoutParams {
		 ...
		 * Call {@link ViewGroup#setLayoutParams(LayoutParams)} after reassigning a new
		 * value to this field.
		 */
		@ViewDebug.ExportedProperty(category = "layout")
		...
		/**
		 * The top margin in pixels of the child. Margin values should be positive. Call
		 * {@link ViewGroup#setLayoutParams(LayoutParams)} after reassigning a new value
		 * to this field.
		 */
		@ViewDebug.ExportedProperty(category = "layout")
		...
		/**
		 * The end margin in pixels of the child. Margin values should be positive. Call
		 * {@link ViewGroup#setLayoutParams(LayoutParams)} after reassigning a new value
		 * to this field.
		 */
		@ViewDebug.ExportedProperty(category = "layout")
		...
		/**
		 * Bit 0: layout direction Bit 1: layout direction Bit 2: left margin undefined
		 * Bit 3: right margin undefined Bit 4: is RTL compatibility mode Bit 5: need
		 * resolution
		 *
		 * Bit 6 to 7 not used
		 *
		...
		@ViewDebug.ExportedProperty(category = "layout", flagMapping = {
				@ViewDebug.FlagToString(mask = LAYOUT_DIRECTION_MASK, equals = LAYOUT_DIRECTION_MASK, name = "LAYOUT_DIRECTION"),
				@ViewDebug.FlagToString(mask = LEFT_MARGIN_UNDEFINED_MASK, equals = LEFT_MARGIN_UNDEFINED_MASK, name = "LEFT_MARGIN_UNDEFINED_MASK"),
				@ViewDebug.FlagToString(mask = RIGHT_MARGIN_UNDEFINED_MASK, equals = RIGHT_MARGIN_UNDEFINED_MASK, name = "RIGHT_MARGIN_UNDEFINED_MASK"),
				@ViewDebug.FlagToString(mask = RTL_COMPATIBILITY_MODE_MASK, equals = RTL_COMPATIBILITY_MODE_MASK, name = "RTL_COMPATIBILITY_MODE_MASK"),
		...
		public MarginLayoutParams(Context c, AttributeSet attrs) {
			...
			setBaseAttributes(a, R.styleable.ViewGroup_MarginLayout_layout_width,
					R.styleable.ViewGroup_MarginLayout_layout_height);

			int margin = a.getDimensionPixelSize(com.android.internal.R.styleable.ViewGroup_MarginLayout_layout_margin,
					-1);
			if (margin >= 0) {
				leftMargin = margin;
				topMargin = margin;
				...
				leftMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginLeft,
						UNDEFINED_MARGIN);
				if (leftMargin == UNDEFINED_MARGIN) {
					mMarginFlags |= LEFT_MARGIN_UNDEFINED_MASK;
					leftMargin = DEFAULT_MARGIN_RESOLVED;
				}
				rightMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginRight,
						UNDEFINED_MARGIN);
				...
				topMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginTop,
						DEFAULT_MARGIN_RESOLVED);
				bottomMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginBottom,
						DEFAULT_MARGIN_RESOLVED);

				startMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginStart,
						DEFAULT_MARGIN_RELATIVE);
				endMargin = a.getDimensionPixelSize(R.styleable.ViewGroup_MarginLayout_layout_marginEnd,
						DEFAULT_MARGIN_RELATIVE);
		 ...
		 * {@link android.view.View#requestLayout()} needs to be done so that the new
		 * margins are taken into account. Left and right margins may be overriden by
		 ...
		 * {@link android.view.View#requestLayout()} needs to be done so that the new
		 * relative margins are taken into account. Left and right margins may be
		 * overriden by {@link android.view.View#requestLayout()} depending on layout
		 ...
		 * 
		 * @param layoutDirection the layout direction. Should be either
		...
		/**
		 * Retuns the layout direction. Can be either {@link View#LAYOUT_DIRECTION_LTR}
		...
		/**
		 * This will be called by {@link android.view.View#requestLayout()}. Left and
		 * Right margins may be overridden depending on layout direction.
		 */
		@Override
		public void resolveLayoutDirection(int layoutDirection) {
			setLayoutDirection(layoutDirection);

			// No relative margin or pre JB-MR1 case or no need to resolve, just dont do
			// anything
		...
		private void doResolveMargins() {
			if ((mMarginFlags & RTL_COMPATIBILITY_MODE_MASK) == RTL_COMPATIBILITY_MODE_MASK) {
				// if left or right margins are not defined and if we have some start or end
				// margin
				...
				// We have some relative margins (either the start one or the end one or both).
				// So use
				// them and override what has been defined for left and right margins. If either
				// start
		...
		public void onDebugDraw(View view, Canvas canvas, Paint paint) {
			...
			fillDifference(canvas, view.getLeft() + oi.left, view.getTop() + oi.top, view.getRight() - oi.right,
					view.getBottom() - oi.bottom, leftMargin, topMargin, rightMargin, bottomMargin, paint);
		}
	 ...
	 *
	 * This code assumes that pointer ids are always in the range 0..31 such that it
	 * can use a bitfield to track which pointer ids are present. As it happens, the
	 * lower layers of the input dispatch pipeline also use the same trick so the
	 * assumption should be safe here...
	 */
	...
	/**
	 * Pooled class that orderes the children of a ViewGroup from start to end based
	 * on how they are laid out and the layout direction.
	 */
	static class ChildListForAccessibility {

		private static final int MAX_POOL_SIZE = 32;

		private static final SynchronizedPool<ChildListForAccessibility> sPool = new SynchronizedPool<ChildListForAccessibility>(
				MAX_POOL_SIZE);
	...
	/**
	 * Pooled class that holds a View and its location with respect to a specified
	 * root. This enables sorting of views based on their coordinates without
	 * recomputing the position relative to the root on every comparison.
	 */
	static class ViewLocationHolder implements Comparable<ViewLocationHolder> {

		private static final int MAX_POOL_SIZE = 32;

		private static final SynchronizedPool<ViewLocationHolder> sPool = new SynchronizedPool<ViewLocationHolder>(
				MAX_POOL_SIZE);
