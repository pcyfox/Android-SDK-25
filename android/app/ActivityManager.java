...
public class ActivityManager {
	...
	@IntDef({ BUGREPORT_OPTION_FULL, BUGREPORT_OPTION_INTERACTIVE, BUGREPORT_OPTION_REMOTE, BUGREPORT_OPTION_WEAR })
	...
	/**
	 * Allows user to monitor progress and enter additional data; might not include
	 * all sections.
	 * 
	 * @hide
	 */
	...
	/**
	 * Takes a bugreport requested remotely by administrator of the Device Owner
	 * app, not the device's user.
	 * 
	 * @hide
	 */
	 ...
	 * <meta-data>}</a> name for a 'home' Activity that declares a package that is
	 * to be uninstalled in lieu of the declaring one. The package named here must
	 * be signed with the same certificate as the one declaring the
	 * {@code <meta-data>}.
	 */
	 ...
	 * voice control when that activity does not support the VOICE category.
	 * 
	 * @hide
	 */
	 ...
	 * not be executed if it is the recipient, and that is indeed the case.
	 * 
	 * @hide
	 */
	 ...
	 * but a task was simply brought to the foreground.
	 * 
	 * @hide
	 */
	...
	/**
	 * Result for IActivityManaqer.startActivity: activity wasn't really started,
	 * but the given Intent was given to the existing top activity.
	 * 
	 * @hide
	 */
	 ...
	 * started while in Lock Task Mode.
	 * 
	 * @hide
	 */
	...
	/**
	 * Flag for IActivityManaqer.startActivity: launch the app for allocation
	 * tracking.
	 * 
	 * @hide
	 */
	...
	/**
	 * Flag for IActivityManaqer.startActivity: launch the app with native debugging
	 * support.
	 * 
	 * @hide
	 */
	...
	/**
	 * Result for IActivityManager.broadcastIntent: trying to send a broadcast to a
	 * stopped user. Fail.
	 * 
	 * @hide
	 */
	...
	/**
	 * @hide User operation call: given user id is the current user, can't be
	 *       stopped.
	 */
	...
	/**
	 * @hide Process is in the background running a service. Unlike oom_adj, this
	...
	/**
	 * @hide Mode for {@link IActivityManager#getAppStartMode}: normal free-to-run
	 *       operation.
	 */
	public static final int APP_START_MODE_NORMAL = 0;

	/**
	 * @hide Mode for {@link IActivityManager#getAppStartMode}: delay running until
	 *       later.
	 */
	public static final int APP_START_MODE_DELAYED = 1;

	/**
	 * @hide Mode for {@link IActivityManager#getAppStartMode}: disable/cancel
	 *       pending launches.
	 */
	...
	/**
	 * Screen compatibility mode: the application can never run in compatibility
	 * mode.
	 * 
	 * @hide
	 */
	...
	public static class StackId {
		...
		/**
		 * Returns true if the activities contained in the input stack display a shadow
		 * around their border.
		 */
		...
		/**
		 * Returns true if the activities contained in the input stack display a decor
		 * view.
		 */
		...
		/**
		 * Returns true if the tasks contained in the stack can be resized independently
		 * of the stack.
		 */
		...
		/**
		 * Returns true if dynamic stacks are allowed to be visible behind the input
		 * stack.
		 */
		...
		/**
		 * Returns true if we try to maintain focus in the current stack when the top
		 * activity finishes.
		 */
		public static boolean keepFocusInStackIfPossible(int stackId) {
			return stackId == FREEFORM_WORKSPACE_STACK_ID || stackId == DOCKED_STACK_ID || stackId == PINNED_STACK_ID;
		}
		...
		public static boolean isResizeableByDockedStack(int stackId) {
			return isStaticStack(stackId) && stackId != DOCKED_STACK_ID && stackId != PINNED_STACK_ID;
		}

		/**
		 * Returns true if the size of tasks in the input stack are affected by the
		 * docked stack changing size.
		 */
		public static boolean isTaskResizeableByDockedStack(int stackId) {
			return isStaticStack(stackId) && stackId != FREEFORM_WORKSPACE_STACK_ID && stackId != DOCKED_STACK_ID
					&& stackId != PINNED_STACK_ID;
		}

		/**
		 * Returns true if the windows of tasks being moved to the target stack from the
		 * source stack should be replaced, meaning that window manager will keep the
		 * old window around until the new is ready.
		 */
		public static boolean replaceWindowsOnTaskMove(int sourceStackId, int targetStackId) {
			return sourceStackId == FREEFORM_WORKSPACE_STACK_ID || targetStackId == FREEFORM_WORKSPACE_STACK_ID;
		}

		/**
		 * Return whether a stackId is a stack containing floating windows. Floating
		 * windows are laid out differently as they are allowed to extend past the
		 * display bounds without overscan insets.
		 */
		...
		/**
		 * Returns true if animation specs should be constructed for app transition that
		 * moves the task to the specified stack.
		 */
		public static boolean useAnimationSpecForAppTransition(int stackId) {

			// TODO: INVALID_STACK_ID is also animated because we don't persist stack id's
			// across
			// reboots.
			return stackId == FREEFORM_WORKSPACE_STACK_ID || stackId == FULLSCREEN_WORKSPACE_STACK_ID
					|| stackId == DOCKED_STACK_ID || stackId == INVALID_STACK_ID;
		}
		...
		/**
		 * Returns true if the top task in the task is allowed to return home when
		 * finished and there are other tasks in the stack.
		 */
		 ...
		 * {@link ActivityOptions#setLaunchBounds} when launching an activity into the
		 * stack.
		 */
		...
		/**
		 * Returns true if any visible windows belonging to apps in this stack should be
		 * kept on screen when the app is killed due to something like the low memory
		 * killer.
		 */
		...
		/**
		 * Returns true if the backdrop on the client side should match the frame of the
		 * window. Returns false, if the backdrop should be fullscreen.
		 */
		...
		/**
		 * Returns true if a window from the specified stack with {@param stackId} are
		 * normally fullscreen, i. e. they can become the top opaque fullscreen window,
		 * meaning that it controls system bars, lockscreen occluded/dismissing state,
		 * screen rotation animation, etc.
		 */
		public static boolean normallyFullscreenWindows(int stackId) {
			return stackId != PINNED_STACK_ID && stackId != FREEFORM_WORKSPACE_STACK_ID && stackId != DOCKED_STACK_ID;
		}

		/**
		 * Returns true if the input stack id should only be present on a device that
		...
		public static boolean isMultiWindowStack(int stackId) {
			return isStaticStack(stackId) || stackId == PINNED_STACK_ID || stackId == FREEFORM_WORKSPACE_STACK_ID
					|| stackId == DOCKED_STACK_ID;
		}

		/**
		 * Returns true if activities contained in this stack can request visible behind
		 * by calling {@link Activity#requestVisibleBehind}.
		 */
		...
		/**
		 * Returns true if this stack may be scaled without resizing, and windows within
		 * may need to be configured as such.
		 */
		...
		/**
		 * Returns true if windows in this stack should be given move animations by
		 * default.
		 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#moveTaskToDockedStack}
	 * which specifies the position of the created docked stack at the top half of
	 * the screen if in portrait mode or at the left half of the screen if in
	 * landscape mode.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#moveTaskToDockedStack}
	 * which specifies the position of the created docked stack at the bottom half
	 * of the screen if in portrait mode or at the right half of the screen if in
	 * landscape mode.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#resizeTask} which
	 * indicates that the resize doesn't need to preserve the window, and can be
	 * skipped if bounds is unchanged. This mode is used by window manager in most
	 * cases.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#resizeTask} which
	 * indicates that the resize should preserve the window if possible.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#resizeTask} which
	 * indicates that the resize should be performed even if the bounds appears
	 * unchanged.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#resizeTask} used by
	 * window manager during a screen rotation.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#resizeTask} used when
	 * the resize is due to a drag action.
	 * 
	 * @hide
	 */
	...
	/**
	 * Input parameter to {@link android.app.IActivityManager#resizeTask} which
	 * indicates that the resize should preserve the window if possible, and should
	 * not be skipped even if the bounds is unchanged. Usually used to force a
	 * resizing when a drag action is ending.
	 * 
	 * @hide
	 */
	...
	/**
	 * Return the approximate per-application memory class of the current device
	 * when an application is running with a large heap. This is the space available
	 * for memory-intensive applications; most applications should not need this
	 * amount of memory, and should instead stay with the {@link #getMemoryClass()}
	 * limit. The returned value is in megabytes. This may be the same size as
	 * {@link #getMemoryClass()} on memory constrained devices, or it may be
	 ...
	 * is ultimately up to the device configuration, but currently it generally
	 * means something in the class of a 512MB device with about a 800x480 or less
	 * screen. This is mostly intended to be used by apps to determine whether they
	...
	/**
	 * Returns true if the system supports at least one form of multi-window. E.g.
	 * freeform, split-screen, picture-in-picture.
	 * 
	 * @hide
	 */
	...
	/**
	 * Information you can set and retrieve about the current activity within the
	 * recent task list.
	 */
	public static class TaskDescription implements Parcelable {
		/** @hide */
		public static final String ATTR_TASKDESCRIPTION_PREFIX = "task_description_";
		private static final String ATTR_TASKDESCRIPTIONLABEL = ATTR_TASKDESCRIPTION_PREFIX + "label";
		private static final String ATTR_TASKDESCRIPTIONCOLOR_PRIMARY = ATTR_TASKDESCRIPTION_PREFIX + "color";
		private static final String ATTR_TASKDESCRIPTIONCOLOR_BACKGROUND = ATTR_TASKDESCRIPTION_PREFIX
				+ "colorBackground";
		private static final String ATTR_TASKDESCRIPTIONICONFILENAME = ATTR_TASKDESCRIPTION_PREFIX + "icon_filename";
		...
		/**
		 * Moves the icon bitmap reference from an actual Bitmap to a file containing
		 * the bitmap.
		 * 
		 * @hide
		 */
		...
		public String toString() {
			return "TaskDescription Label: " + mLabel + " Icon: " + mIcon + " IconFilename: " + mIconFilename
	...
	public static class RecentTaskInfo implements Parcelable {
		/**
		 * If this task is currently running, this is the identifier for it. If it is
		 * not running, this will be -1.
		 */
		public int id;
		 ...
		 * that was initially started; the component of the baseIntent in this case is
		 * the name of the actual activity implementation that the alias referred to.
		...
		/**
		 * The recent activity values for the highest activity in the stack to have set
		 * the values.
	...
	/**
	 * Flag for use with {@link #getRecentTasks}: return all tasks, even those that
	 * have set their
	 ...
	 *
	 * <p>
	 * <b>Note: this method is only intended for debugging and presenting task
	 * management user interfaces</b>. This should never be used for core logic in
	 * an application, such as deciding between different behaviors based on the
	 * information found here. Such uses are <em>not</em> supported, and will likely
	 * break in the future. For example, if multiple applications can be actively
	 * running at the same time, assumptions made about the meaning of the data here
	 * for purposes of control flow will be incorrect.
	 * </p>
	 *
	 * @deprecated As of {@link android.os.Build.VERSION_CODES#LOLLIPOP}, this
	 *             method is no longer available to third party applications: the
	 *             introduction of document-centric recents means it can leak
	 *             personal information to the caller. For backwards compatibility,
	 *             it will still return a small subset of its data: at least the
	 *             caller's own tasks (though see {@link #getAppTasks()} for the
	 *             correct supported way to retrieve that information), and possibly
	 *             some other tasks such as home that are known to not be sensitive.
	 *
	 * @param maxNum
	 *            The maximum number of entries to return in the list. The actual
	 *            number returned may be smaller, depending on how many tasks the
	 *            user has started and the maximum number the system can remember.
	 * @param flags
	 *            Information about what to return. May be any combination of
	 *            {@link #RECENT_WITH_EXCLUDED} and
	 *            {@link #RECENT_IGNORE_UNAVAILABLE}.
	 *
	 * @return Returns a list of RecentTaskInfo records describing each of the
	 *         recent tasks.
	 */
	@Deprecated
	 ...
	 * specific user. It requires holding the
	 ...
	 *
	 * @return Returns a list of RecentTaskInfo records describing each of the
	 *         recent tasks. Most recently activated tasks go first.
	 *
	 * @hide
	 */
	 ...
	 * task actually has a process it is actively running in; it simply means that
	 * the user has gone to it and never closed it, but currently the system may
	 * have killed its process and is only holding on to its last state in order to
	...
	public static class RunningTaskInfo implements Parcelable {
		...
		/**
		 * Number of activities that are currently running (not stopped and persisted)
		 * in this task.
		 */
	 ...
	 *
	 * @param activity
	 *            The activity that is adding the entry. This is used to help
	 *            determine the context that the new recents entry will be in.
	 * @param intent
	 *            The Intent that describes the recents entry. This is the same
	 *            Intent that you would have used to launch the activity for it. In
	 *            generally you will want to set both
	 *            {@link Intent#FLAG_ACTIVITY_NEW_DOCUMENT} and
	 *            {@link Intent#FLAG_ACTIVITY_RETAIN_IN_RECENTS}; the latter is
	 *            required since this recents entry will exist without an activity,
	 *            so it doesn't make sense to not retain it when its activity
	 *            disappears. The given Intent here also must have an explicit
	 *            ComponentName set on it.
	 * @param description
	 *            Optional additional description information.
	 * @param thumbnail
	 *            Thumbnail to use for the recents entry. Should be the size given
	 *            by {@link #getAppTaskThumbnailSize()}. If the bitmap is not that
	 *            exact size, it will be recreated in your process, probably in a
	 *            way you don't like, before the recents entry is added.
	 *
	 * @return Returns the task id of the newly added app task, or -1 if the add
	 *         failed. The most likely cause of failure is that there is no more
	 *         room for more tasks for your app.
	 */
	public int addAppTask(@NonNull Activity activity, @NonNull Intent intent, @Nullable TaskDescription description,
		...
		try {
			return ActivityManagerNative.getDefault().addAppTask(activity.getActivityToken(), intent, description,
					thumbnail);
		} catch (RemoteException e) {
			throw e.rethrowFromSystemServer();
		}
	 ...
	 *             information to the caller. For backwards compatibility, it will
	 *             still retu rn a small subset of its data: at least the caller's
	 *             own tasks, and possibly some other tasks such as home that are
	 ...
	 *            user has started.
	 *
	 * @return Returns a list of RunningTaskInfo records describing each of the
	 *         running tasks.
	 */
	@Deprecated
	...
	public static class TaskThumbnailInfo implements Parcelable {
		...
		private static final String ATTR_TASK_WIDTH = ATTR_TASK_THUMBNAILINFO_PREFIX + "task_width";
		private static final String ATTR_TASK_HEIGHT = ATTR_TASK_THUMBNAILINFO_PREFIX + "task_height";
		private static final String ATTR_SCREEN_ORIENTATION = ATTR_TASK_THUMBNAILINFO_PREFIX + "screen_orientation";
	...
	/**
	 * Flag for {@link #moveTaskToFront(int, int)}: also move the "home" activity
	 * along with the task, so it is positioned immediately behind the task.
	 */
	...
	/**
	 * Equivalent to calling {@link #moveTaskToFront(int, int, Bundle)} with a null
	 * options argument.
	 *
	 * @param taskId
	...
	/**
	 * Ask that the task associated with a given task ID be moved to the front of
	 * the stack, so it is now visible to the user. Requires that the caller hold
	 * permission {@link android.Manifest.permission#REORDER_TASKS} or a
	 ...
	 *            {@link #MOVE_TASK_WITH_HOME}, {@link #MOVE_TASK_NO_USER_ACTION}.
	 * @param options
	 *            Additional options for the operation, either null or as per
	 *            {@link Context#startActivity(Intent, android.os.Bundle)
	...
	public static class RunningServiceInfo implements Parcelable {
		...
		/**
		 * The time when the service was first made active, either by someone starting
		 * or binding to it. This is in units of
		...
		/**
		 * Number of times the service's process has crashed while the service is
		 * running.
		 */
		public int crashCount;

		/**
		 * The time when there was last activity in the service (either explicit
		 * requests to start it or clients binding to it). This is in units of
		 * {@link android.os.SystemClock#uptimeMillis()}.
		 */
		public long lastActivityTime;

		/**
		 * If non-zero, this service is not currently running, but scheduled to restart
		 * at the given time.
		 */
		...
		/**
		 * Bit for {@link #flags}: set if the service has asked to run as a foreground
		 * process.
		 */
		public static final int FLAG_FOREGROUND = 1 << 1;

		/**
		 * Bit for {@link #flags): set if the service is running in a core system
		 * process.
		 */
		public static final int FLAG_SYSTEM_PROCESS = 1 << 2;

		/**
		 * Bit for {@link #flags): set if the service is running in a persistent
		 * process.
		 */
		...
		/**
		 * For special services that are bound to by system code, this is the package
	 ...
	 *
	 * <p>
	 * <b>Note: this method is only intended for debugging or implementing service
	 * management type user interfaces.</b>
	 * </p>
	 *
	 * @param maxNum
	 *            The maximum number of entries to return in the list. The actual
	 *            number returned may be smaller, depending on how many services are
	 *            running.
	 *
	 * @return Returns a list of RunningServiceInfo records describing each of the
	 *         running tasks.
	 */
	...
	public static class MemoryInfo implements Parcelable {
		 ...
		 * the device, not including below-kernel fixed allocations like DMA buffers,
		 * RAM for the baseband CPU, etc.
		 */
		public long totalMem;

		/**
		 * The threshold of {@link #availMem} at which we consider memory to be low and
		 * start killing background services and other non-extraneous processes.
		 */
	 ...
	 * used to help decide how to manage your own memory, though note that polling
	 * is not recommended and
	 * {@link android.content.ComponentCallbacks2#onTrimMemory(int)
	 * ComponentCallbacks2.onTrimMemory(int)} is the preferred way to do this. Also
	 * see {@link #getMyMemoryState} for how to retrieve the current trim level of
	 * your process as needed, which gives a better hint for how to manage its
	 * memory.
	 */
	...
	public static class StackInfo implements Parcelable {
		...
		public void readFromParcel(Parcel source) {
			stackId = source.readInt();
			bounds = new Rect(source.readInt(), source.readInt(), source.readInt(), source.readInt());
	...
	public boolean clearApplicationUserData(String packageName, IPackageDataObserver observer) {
		try {
			return ActivityManagerNative.getDefault().clearApplicationUserData(packageName, observer,
					UserHandle.myUserId());
		} catch (RemoteException e) {
			throw e.rethrowFromSystemServer();
		}
	 ...
	 *
	 * @return {@code true} if the application successfully requested that the
	 *         application's data be erased; {@code false} otherwise.
	 */
	...
	/**
	 * Permits an application to get the persistent URI permissions granted to
	 * another.
	 *
	 * <p>
	...
	/**
	 * Permits an application to clear the persistent URI permissions granted to
	 * another.
	 *
	 * <p>
	 * Typically called by Settings.
	 ...
	 *
	 * @return Returns a list of ProcessErrorStateInfo records, or null if there are
	 *         no current error conditions (it will not return an empty list). This
	...
	public static class RunningAppProcessInfo implements Parcelable {
		...
		/**
		 * Constant for {@link #flags}: this is an app that is unable to correctly save
		 * its state when going to the background, so it can not be killed while in the
		 * background.
		 * 
		 * @hide
		 */
		...
		/**
		 * Constant for {@link #flags}: this process is associated with a persistent
		 * system app.
		 * 
		 * @hide
		 */
		...
		/**
		 * Flags of information. May be any of {@link #FLAG_CANT_SAVE_STATE}.
		 * 
		 * @hide
		 */
		public int flags;
		 ...
		 * supplied to {@link android.content.ComponentCallbacks2#onTrimMemory(int)
		 * ComponentCallbacks2.onTrimMemory(int)}.
		 */
		public int lastTrimLevel;

		/**
		 * Constant for {@link #importance}: This process is running the foreground UI;
		 * that is, it is the thing currently at the top of the screen that the user is
		 * interacting with.
		 */
		 ...
		 * service, for example to perform music playback even while the user is not
		 * immediately in the app. This generally indicates that the process is doing
		 * something the user actively cares about.
		 */
		public static final int IMPORTANCE_FOREGROUND_SERVICE = 125;

		/**
		 * Constant for {@link #importance}: This process is running the foreground UI,
		 * but the device is asleep so it is not visible to the user. This means the
		 * user is not really aware of the process, because they can not see or interact
		 * with it, but it is quite important because it what they expect to return to
		 * once unlocking the device.
		 */
		public static final int IMPORTANCE_TOP_SLEEPING = 150;

		/**
		 * Constant for {@link #importance}: This process is running something that is
		 * actively visible to the user, though not in the immediate foreground. This
		 * may be running a window that is behind the current foreground (so paused and
		 * with its state saved, not interacting with the user, but visible to them to
		 * some degree); it may also be running other services under the system's
		 * control that it inconsiders important.
		 */
		public static final int IMPORTANCE_VISIBLE = 200;

		/**
		 * Constant for {@link #importance}: This process is not something the user is
		 * directly aware of, but is otherwise perceptable to them to some degree.
		 */
		public static final int IMPORTANCE_PERCEPTIBLE = 130;

		/**
		 * Constant for {@link #importance}: This process is running an application that
		 * can not save its state, and thus can't be killed while in the background.
		 * 
		 * @hide
		 */
		...
		/**
		 * Constant for {@link #importance}: This process is contains services that
		 * should remain running. These are background services apps have started, not
		 * something the user is aware of, so they may be killed by the system
		 * relatively freely (though it is generally desired that they stay running as
		 * long as they want to).
		 */
		public static final int IMPORTANCE_SERVICE = 300;

		/**
		 * Constant for {@link #importance}: This process process contains background
		 * code that is expendable.
		 */
		public static final int IMPORTANCE_BACKGROUND = 400;

		/**
		 * Constant for {@link #importance}: This process is empty of any actively
		 * running code.
		 */
		 ...
		 * one of {@link #IMPORTANCE_FOREGROUND}, {@link #IMPORTANCE_VISIBLE},
		 * {@link #IMPORTANCE_SERVICE}, {@link #IMPORTANCE_BACKGROUND}, or
		 * {@link #IMPORTANCE_EMPTY}. These constants are numbered so that "more
		 * important" values are always smaller than "less important" values.
		 */
		public int importance;

		/**
		 * An additional ordering within a particular {@link #importance} category,
		 * providing finer-grained information about the relative utility of processes
		 * within a category. This number means nothing except that a smaller values are
		 * more recently used (and thus more important). Currently an LRU value is only
		 * maintained for the {@link #IMPORTANCE_BACKGROUND} category, though others may
		 * be maintained in the future.
		 */
		public int lru;

		/**
		 * Constant for {@link #importanceReasonCode}: nothing special has been
		 * specified for the reason for this level.
		 */
		public static final int REASON_UNKNOWN = 0;

		/**
		 * Constant for {@link #importanceReasonCode}: one of the application's content
		 * providers is being used by another process. The pid of the client process is
		 * in {@link #importanceReasonPid} and the target provider in this process is in
		...
		/**
		 * For the specified values of {@link #importanceReasonCode}, this is the
		 * process ID of the other process that is a client of this process. This will
		 * be 0 if no other process is using this one.
		 */
		public int importanceReasonPid;

		/**
		 * For the specified values of {@link #importanceReasonCode}, this is the name
		 * of the component that is being used in this process.
		 */
		public ComponentName importanceReasonComponent;

		/**
		 * When {@link #importanceReasonPid} is non-0, this is the importance of the
		 * other pid. @hide
		 */
	 ...
	 *
	 * <p>
	 * <b>Note: this method is only intended for debugging or building a user-facing
	 * process management UI.</b>
	 * </p>
	 *
	 * @return Returns a list of ApplicationInfo records, or null if none This list
	...
	/**
	 * Sets the memory trim mode for a process and schedules a memory trim
	 * operation.
	 *
	 * <p>
	 * <b>Note: this method is only intended for testing framework.</b>
	 * </p>
	 *
	 ...
	 * </p>
	 *
	 * @return Returns a list of RunningAppProcessInfo records, or null if there are
	...
	/**
	 * Return the importance of a given package name, based on the processes that
	 * are currently running. The return value is one of the importance constants
	 * defined in {@link RunningAppProcessInfo}, giving you the highest importance
	 * of all the processes that this package has code running inside of. If there
	 * are no processes running its code,
	 * {@link RunningAppProcessInfo#IMPORTANCE_GONE} is returned.
	 * 
	 * @hide
	 */
	@SystemApi
	 ...
	 * fill in all fields of the {@link RunningAppProcessInfo}. The only fields that
	 * will be filled in are {@link RunningAppProcessInfo#pid},
	 * {@link RunningAppProcessInfo#uid},
	 * {@link RunningAppProcessInfo#lastTrimLevel},
	 * {@link RunningAppProcessInfo#importance}, {@link RunningAppProcessInfo#lru},
	 * and {@link RunningAppProcessInfo#importanceReasonCode}.
	 */
	 ...
	 * @return Returns an array of memory information, one for each requested pid.
	 */
	 ...
	 *             {@link #killBackgroundProcesses(String)}; the previous behavior
	 *             here is no longer available to applications because it allows
	 *             them to break other applications by removing their alarms,
	 ...
	 * reclaim memory; the system will take care of restarting these processes in
	 ...
	 * {@link Intent#ACTION_PACKAGE_RESTARTED} broadcast will be sent, so that any
	 * of its registered alarms can be stopped, notifications removed, etc.
	 *
	 * <p>
	 * You must hold the permission
	 * {@link android.Manifest.permission#FORCE_STOP_PACKAGES} to be able to call
	 * this method.
	 *
	 * @param packageName
	 ...
	 *
	 * @hide This is not available to third party applications due to it allowing
	 *       them to break other applications by stopping their services, removing
	 *       their alarms, etc.
	 */
	 ...
	 */
	/*
	 * public Map<String, Integer> getAllPackageLaunchCounts() { try { IUsageStats
	 * usageStatsService = IUsageStats.Stub.asInterface(
	 * ServiceManager.getService("usagestats")); if (usageStatsService == null) {
	 * return new HashMap<String, Integer>(); }
	 * 
	 * UsageStats.PackageStats[] allPkgUsageStats =
	 * usageStatsService.getAllPkgUsageStats( ActivityThread.currentPackageName());
	 * if (allPkgUsageStats == null) { return new HashMap<String, Integer>(); }
	 * 
	 * Map<String, Integer> launchCounts = new HashMap<String, Integer>(); for
	 * (UsageStats.PackageStats pkgUsageStats : allPkgUsageStats) {
	 * launchCounts.put(pkgUsageStats.getPackageName(),
	 * pkgUsageStats.getLaunchCount()); }
	 * 
	 * return launchCounts; } catch (RemoteException e) { Log.w(TAG,
	 * "Could not query launch counts", e); return new HashMap<String, Integer>(); }
	 * }
	 */
	...
	public static int checkComponentPermission(String permission, int uid, int owningUid, boolean exported) {
		...
		if (!exported) {
			 ...
			 * here.fillInStackTrace(); Slog.w(TAG,
			 * "Permission denied: checkComponentPermission() owningUid=" + owningUid,
			 * here);
			 */
			return PackageManager.PERMISSION_DENIED;
		}
	...
	/**
	 * @hide Helper for dealing with incoming user arguments to system service
	 *       calls. Takes care of checking permissions and converting USER_CURRENT
	 *       to the actual current user.
	 *
	 * @param callingPid
	 ...
	 *            {@link android.Manifest.permission#INTERACT_ACROSS_USERS_FULL} to
	 *            be able to run as a different user than their current process;
	 *            otherwise they must hold
	 *            {@link android.Manifest.permission#INTERACT_ACROSS_USERS}.
	 * @param name
	 *            Optional textual name of the incoming call; only for generating
	 *            error messages.
	 * @param callerPackage
	 *            Optional package name of caller; only for error messages.
	 *
	 * @return Returns the user ID that the call should run as. Will always be a
	 *         concrete user number, unless <var>allowAll</var> is true in which
	 *         case it could also be USER_ALL.
	 */
	public static int handleIncomingUser(int callingPid, int callingUid, int userId, boolean allowAll,
		...
		try {
			return ActivityManagerNative.getDefault().handleIncomingUser(callingPid, callingUid, userId, allowAll,
	...
	/**
	 * Logs out current current foreground user by switching to the system user and
	 * stopping the user being switched from.
	 * 
	 * @hide
	 */
	...
	/**
	 * Return whether the given user is actively running. This means that the user
	 * is in the "started" state, not "stopped" -- it is currently allowed to run
	 * code through scheduled alarms, receiving broadcasts, etc. A started user may
	 * be either the current foreground user or a background user; the result here
	 ...
	 *            descriptor is <em>not</em> closed by this function; the caller
	 *            continues to own it.
	 * @param packageName
	...
	/**
	 * Request that the system start watching for the calling process to exceed a
	 * pss size as given here. Once called, the system will look for any occasions
	 * where it sees the associated process with a larger pss size and, when this
	 * happens, automatically pull a heap dump from it and allow the user to share
	 * the data. Note that this request continues running even if the process is
	 * killed and restarted. To remove the watch, use
	 * {@link #clearWatchHeapLimit()}.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Callers can optionally implement {@link #ACTION_REPORT_HEAP_LIMIT} to
	 * directly handle heap limit reports themselves.
	 * </p>
	 *
	 * @param pssSize
	 ...
	 * {@link #setWatchHeapLimit(long)}. If your package has an activity handling
	 * this action, it will be launched with the heap data provided to it the same
	 * way as {@link Intent#ACTION_SEND}. Note that to match the activty must
	 * support this action and a MIME type of "*&#47;*".
	 */
	...
	/**
	 * Return whether currently in lock task mode. When in this mode no new tasks
	 * can be created or switched to.
	 *
	 ...
	 * {@link #LOCK_TASK_MODE_NONE}, {@link #LOCK_TASK_MODE_LOCKED} and
	 ...
	 * thread may be subject to restrictions on the amount of time it can run.
	 *
	...
	/**
	 * The AppTask allows you to manage your own application's tasks. See
	 * {@link android.app.ActivityManager#getAppTasks()}
	 */
	public static class AppTask {
		...
		/**
		 * Finishes all activities in this task and removes it from the recent tasks
		 * list.
		 */
		 ...
		 *
		 * @return The RecentTaskInfo for this task, or null if the task no longer
		 *         exists.
		 */
		...
		/**
		 * Start an activity in this task. Brings the task to the foreground. If this
		 * task is not currently active (that is, its id < 0), then a new activity for
		 * the given Intent will be launched as the root of the task and the task
		 * brought to the foreground. Otherwise, if this task is currently active and
		 * the Intent does not specify an activity to launch in a new task, then a new
		 * activity for the given Intent will be launched on top of the task and the
		 * task brought to the foreground. If this task is currently active and the
		 * Intent specifies {@link Intent#FLAG_ACTIVITY_NEW_TASK} or would otherwise be
		 * launched in to a new task, then the activity not launched but this task be
		 * brought to the foreground and a new intent delivered to the top activity if
		 * appropriate.
		 *
		 * <p>
		 * In other words, you generally want to use an Intent here that does not
		 * specify {@link Intent#FLAG_ACTIVITY_NEW_TASK} or
		 * {@link Intent#FLAG_ACTIVITY_NEW_DOCUMENT}, and let the system do the right
		 * thing.
		 * </p>
		 *
		 * @param intent
		...
		public void startActivity(Context context, Intent intent, Bundle options) {
			...
			thread.getInstrumentation().execStartActivityFromAppTask(context, thread.getApplicationThread(),
		 ...
		 *
		 * @param exclude
		 *            If true, {@link Intent#FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS} will be
		 *            set; otherwise, it will be cleared.
		 */
